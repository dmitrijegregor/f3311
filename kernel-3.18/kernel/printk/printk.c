...
extern int debugcontrol_uart_disable; //add by sony MTBF
...
static void call_console_drivers(int level, const char *text, size_t len)
{
	...
	for_each_console(con) {
		if ((printk_disable_uart & debugcontrol_uart_disable) && (con->flags & CON_CONSDEV))
			continue;
...
static int store_log_file(unsigned long long now_time)
{
	...
	while (log_seq < console_seq) {
		...
		if (msg->ts_nsec > now_time) {
			...
			if (__LOG_BUF_LEN <= log_count + count)
				break;
