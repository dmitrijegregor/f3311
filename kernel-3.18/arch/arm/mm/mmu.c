...
static void __init create_mapping(struct map_desc *md)
{
	...
	if (md->virtual != vectors_base() && md->virtual < TASK_SIZE) {
		printk(KERN_WARNING "CCIKLOG: vectors_base()=0x%0811x, TASK_SIZE=%lu", vectors_base(), TASK_SIZE);
...
printk(">>>create_mapping start");
	...
	MTK_MEMCFG_LOG_AND_PRINTK(KERN_ALERT
			"[CCIKLOG]0x%08llX - 0x%08llX (0x%llX) : 0x%lX\n",
...
// Need implement here
	...
	map.pfn = __phys_to_pfn(CCI_KLOG_START_ADDR_PHYSICAL);
	map.virtual = __phys_to_virt(CCI_KLOG_START_ADDR_PHYSICAL);
	map.length = CCI_KLOG_SIZE;
	map.type = MT_MEMORY_RW;

	create_mapping(&map);

	memblock_set_current_limit(CCI_KLOG_START_ADDR_PHYSICAL+CCI_KLOG_SIZE);
