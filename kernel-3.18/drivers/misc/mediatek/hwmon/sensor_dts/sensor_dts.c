...
/*----------------ALSPS STK---------------------*/
	...
	printk("stk Nina in get_alsps_dts_func\n ");
	...
	{
		hw->i2c_num	=	i2c_num[0];
		printk("stk Nina in sensor_dts.c hw->i2c_num=%d\n",hw->i2c_num);
	}
	...
	if (ret == 0) {
		for (i = 0; i < C_CUST_I2C_ADDR_NUM; i++)
			{hw->i2c_addr[i]   = i2c_addr[i];
			printk("stk in sensor_dts.c hw->i2c_addr[%d] =%d\n",i,hw->i2c_addr[i]);
			}
	}
		...
		for (i = 0; i < ARRAY_SIZE(als_level); i++)
			{
				...
				printk("stk in sensor_dts.c hw->als_level[%d] =%d\n",i,hw->als_level[i]);
			}
	}
		...
		for (i = 0; i < ARRAY_SIZE(als_value); i++)
			{
				...
				printk("stk Nina in sensor_dts.c hw->als_value[%d] =%d\n",i,hw->als_value[i]);
			}
	}
