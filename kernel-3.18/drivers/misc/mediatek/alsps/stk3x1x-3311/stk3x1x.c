...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	450
	#define STK_LT_N_CT	     270
...
//CCI Colby add start
unsigned int cci_transmittance_cali = 1100;
...
//add for ALS cali
    ...
    if(((alscode)*cci_transmittance_cali) > 72088500){  //(((*data)*cci_transmittance_cali)/1100) > 65535
		...
		alscode = ((alscode)*cci_transmittance_cali)/1100;
	}
	else
		return 0;
    	}
    	if(CCI_debug_for_stk3x1x)
        APS_DBG("stk cci_transmittance_cali = %d, alscode = %05d.", cci_transmittance_cali,alscode);
...
static uint32_t stk3x1x_get_als_reading_AVG(int sSampleNo){
	...
	while(DataCount < sSampleNo)
	{
		...
		if((err = stk3x1x_read_als(stk3x1x_obj->client, &stk3x1x_obj->als))){
			APS_ERR("[#23]%s: stk3x1x_read_als adc fail\n", __func__);
			}
		else{
			ALSData = stk3x1x_obj->als;
			//APS_ERR("[#23]%s: als adc code = %d\n", __func__, ALSData);
			sAveAlsData +=  ALSData;
			DataCount++;
			}
	}
...
//CCI Colby add start 20131022
//add for ALS cali
int stk3x1x_read_als_avg(int sSampleNo)
{
    int ALSData = 0;
    int DataCount = 0;
    int sAveAlsData = 0;
...
int stk3x1x_cci_als_cali(int sSampleNo)
{
  //  int res;
    ...
    als_reading = stk3x1x_read_als_avg(sSampleNo);
    ...
    cci_transmittance_cali = (500*cci_transmittance_cali)/als_reading;
    cci_als_adc_cali = als_reading;
    if(cci_transmittance_cali == 0)
        cci_transmittance_cali = 1100;
    APS_ERR("[#23]stk3x1x_cci_als_cali() cci_transmittance_cali = %d, als_reading = %d\n", cci_transmittance_cali, als_reading);
...
int stk3x1x_write_int(struct i2c_client *client, u8 data)
{
	struct stk3x1x_priv *obj = i2c_get_clientdata(client);        
	int ret = 0;
	APS_LOG("stk obj->addr.intmode = %d,\n",obj->addr.intmode);
...
{
	struct stk3x1x_priv *obj = i2c_get_clientdata(client);
	int err=0, cur = 0, old = atomic_read(&obj->state_val);
	int trc = atomic_read(&obj->trace);
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable)
{
	...
	cur = old;	
#ifdef STK_TUNE0
//	APS_LOG("%s: stk3x1x_enable_ps() enter enable=%d\n", __FUNCTION__, enable);
// CCI add First boot H/L thd for debug start
	if(enable){
...
*/		
	...
	else //STK add for CCI start 20130430 dust issue for variation in 2 consequential calls 
	{
		...
		{
			if((2*fac_ct-(cci_ps_low_thd-fac_ct)) > hang_up_hthd)
			{
				APS_ERR("%s: [Colby] use CCI calibration value: cci_ps_high_thd = %d, cci_ps_low_thd = %d\n", __FUNCTION__, cci_ps_high_thd, cci_ps_low_thd);
				atomic_set(&stk3x1x_obj->ps_high_thd_val, 2*fac_ct - (cci_ps_low_thd-fac_ct)); 
				atomic_set(&stk3x1x_obj->ps_low_thd_val, 2*fac_ct - (cci_ps_high_thd-fac_ct)); 
			}
			else
			{
				APS_ERR("%s: [Colby] use CCI calibration value: cci_ps_high_thd = %d, cci_ps_low_thd = %d\n", __FUNCTION__, cci_ps_high_thd, cci_ps_low_thd);
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	//diff > temp_max_min_diff :avoid pre pressed before P sensor enable
	if(diff > STK_MAX_MIN_DIFF && diff > temp_max_min_diff)
	{
		...
		else
		{
			...
			printk(KERN_INFO "%s:MIN Compare psi_set(%d) and MAX_CT(%d)\n", __func__, obj->psi_set, MAX_CT);
			obj->psi_set = MIN_COMPARE(obj->psi_set, MAX_CT);
			printk(KERN_INFO "%s:The Result CT: psi_set = %d\n", __func__, obj->psi_set);
			//printk(KERN_INFO "%s:=========================END========================\n", __func__);
			
			APS_ERR(" [stk] use CCI calibration value: cci_ps_high_thd = %d, cci_ps_low_thd = %d\n", cci_ps_high_thd, cci_ps_low_thd);
			atomic_set(&stk3x1x_obj->ps_high_thd_val, obj->psi_set + cci_ps_high_thd-fac_ct); 
			//atomic_set(&stk3x1x_obj->ps_low_thd_val, obj->psi_set + cci_ps_low_thd-fac_ct);
			atomic_set(&stk3x1x_obj->ps_low_thd_val, obj->psi_set +((cci_ps_low_thd - fac_ct)*135/100+fac_ct) -fac_ct);
		}
		//STK add for CCI end 20130112					
		if((ret = stk3x1x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_high_thd_val))))
		{
...
/*
*P sensor calibration funtion
*stk_tune_zero_get_ps_data => Default k: every first_boot
*stk_ps_tune_zero_func_fae => Auto k: every phone call ,prevent phone has oil
*/
...
static void stk3x1x_eint_work(struct work_struct *work)
{
	...
	if(((1<<STK_BIT_ALS) & obj->pending_intr) && (obj->hw->polling_mode_als == 0))
	{
		...
		near_far_state=stk3x1x_get_als_value(obj, obj->als);
...
{	
	...
	if (ps_data->irq_node) {
		of_property_read_u32_array(ps_data->irq_node, "debounce", ints,
					   ARRAY_SIZE(ints));
		gpio_request(ints[0], "stk-int");
...
static ssize_t stk3x1x_show_als(struct device_driver *ddri, char *buf)
{
	...
	if(!stk3x1x_obj)
	{
		APS_ERR("stk3x1x_obj is null!!\n");
		//return 0;
		return scnprintf(buf, PAGE_SIZE, "ERROR:stk3x1x_obj is null\n");
	}
...
#ifdef STK_TUNE0
/*
*Resort P sensor data from nvram ,and check  factory calibration values with or without
*/
static ssize_t stk3x1x_store_cali(struct device_driver *ddri, const char *buf,size_t size)
{
	...
	// = ;
	//stk3x1x_obj->ps_low_thd_val = cci_result_ct_x2 + (cci_ps_low_thd - cci_result_ct);	
	atomic_set(&stk3x1x_obj->ps_high_thd_val, cci_result_ct_x2 + (cci_ps_high_thd - cci_result_ct));
	atomic_set(&stk3x1x_obj->ps_low_thd_val, cci_result_ct_x2 + (cci_ps_low_thd - cci_result_ct));
	high_thd=cci_result_ct_x2 + (cci_ps_high_thd - cci_result_ct);
	low_thd=cci_result_ct_x2 + (cci_ps_low_thd - cci_result_ct);
/*Nina need check */
	...
	printk(KERN_INFO "%s: set HT=%d,LT=%d according to cci_result_ct_x2\n", __func__, (cci_result_ct_x2 + (cci_ps_high_thd - cci_result_ct)),  (cci_result_ct_x2 + (cci_ps_low_thd - cci_result_ct)));				
	...
	if(cci_transmittance_cali ==0)
		 cci_transmittance_cali=1100; // writ back to als_transmittance
...
{
	APS_ERR("[#23]stk3x1x_show_nvdata\n");
	APS_ERR("[#23]cci_ps_high_thd = %d\n", cci_ps_high_thd);
	APS_ERR("[#23]cci_ps_low_thd =  %d\n", cci_ps_low_thd);
	APS_ERR("[#23]cci_result_ct = %d\n", cci_result_ct);
	APS_ERR("[#23]cci_ps_cover =  %d\n", cci_ps_cover);
	APS_ERR("[#23]cci_als_value =  %d\n", cci_als_value);
...
static uint32_t stk3x1x_get_ps_reading_AVG(int sSampleNo){
	...
	while(DataCount < sSampleNo)
	{
		...
		if(err!=0)
			{
			APS_ERR("[#23]%s: stk3x1x_read_ps fail\n", __func__);
			}
		else{
			PSData = stk3x1x_obj->ps;
			APS_ERR("[#23]%s: ps code = %d\n", __func__, PSData);
			sAvePsData +=  PSData;
			DataCount++;
			}
	}
...
static ssize_t stk3x1x_show_GC_20mm(struct device_driver *ddri, char *buf)
{
	...
	if(!stk3x1x_obj)
	{
		APS_ERR("[#23]STK stk3x1x_obj is null!!\n");
		return 0;
	}
	...
	{
		APS_ERR("[#23]write low thd error: %d\n", err);
	...
	{
		APS_ERR("[#23]write high thd error: %d\n", err);
	...
	APS_ERR("[#23]%s:Start calibrating with Gray Card 2.0 cm...\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5);

	cci_ps_high_thd = reading; //*****************This is F2N threshold***************
	APS_ERR("[#23]%s:calibrating with Gray Card 2.0c m done!!!!! cci_ps_high_thd = %d\n", __func__, cci_ps_high_thd);
...
static ssize_t stk3x1x_show_GC_30mm(struct device_driver *ddri, char *buf)
{
	...
	//get ps code for 5 times
	APS_ERR("[#23]%s:Start calibrating with Gray Card 3.0 cm...\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5);

	cci_ps_low_thd = reading; //*****************This is N2F threshold***************
	APS_ERR("[#23]%s:calibrating with Gray Card 3.0c m done!!!!! cci_ps_low_thd = %d\n", __func__, cci_ps_low_thd);
...
static ssize_t stk3x1x_show_GC_ct(struct device_driver *ddri, char *buf)
{
	...
	bool result = false;
	//get ps code for 5 times
	APS_ERR("[#23]%s:Start calibrating CT...\n", __func__);
	reading = stk3x1x_get_ps_reading_AVG(5);
	...
	if (cci_result_ct > 0) {
		...
		if(hwid>=4){
		//if((Diff_ThdL_CT > 200)&&(cci_ps_high_thd > cci_ps_low_thd)&&(cci_result_ct < 2200)){
		if((Diff_ThdL_CT > 50)&&(cci_ps_high_thd > cci_ps_low_thd)&&(cci_result_ct < 1300)){//2016-3-10 modified
			...
			result = true;
			APS_ERR("[#23]---------------Test PS Gray card CT successfully----------------\n");
			APS_ERR("[#23]cci_ps_low_thd = %d\n", cci_ps_low_thd);
		} else {

			APS_ERR("[#23]---------------Test PS Gray card CT fail----------------\n");
			APS_ERR("[#23]cci_ps_high_thd = %d cci_ps_low_thd = %d cci_result_ct = %d \n",cci_ps_high_thd, cci_ps_low_thd,cci_result_ct);
...
static ssize_t stk3x1x_show_test_GC(struct device_driver *ddri, char *buf)
{
	...
	APS_ERR("%s:[#23]ps_CCI_test_GC_show() CCI_FTM_INTERRUPT = %d\n", __func__, CCI_FTM_INTERRUPT);
	APS_ERR("[#23] adc= %d\n",test_ps_code);
...
static ssize_t als_CCI_cali_Light_show(struct device_driver *ddri, char *buf)
{
	...
	if( ret && cci_als_adc_cali >= 152 && cci_als_adc_cali <= 867 ){
...
static ssize_t cci_dummy_lux_store(struct device_driver *ddri, const char *buf, size_t count)
{
       int value = -1;
       int ret;
...
//CCI Colby add end 20130111
//add PL sensor calibration device node start 20140418
...
//add PL sensor calibration device node end 20140418
//CCI Colby add start 20130312
...
static int als_get_data(int *value, int *status)
{
	...
	if (als_reading<0) {
		...
		reading_lux = stk3x1x_get_als_value(stk3x1x_obj, als_reading);
		APS_DBG("stk3x1x als_get_data Light lux = %d,,value= %d ,status = %d ,als_reading= %d\n",reading_lux,*value,*status,als_reading);
		//if(abs(stk3x1x_ptr->als_lux_last - reading_lux) >= STK_ALS_CHANGE_THD)
		//{
			*value=reading_lux;
			//stk3x1x_obj->als_lux_last = reading_lux;
			if (*value < 0)
				err = -1;
...
{
	int err = 0;
//	APS_LOG("stk from alsps get ps data");
	if (!stk3x1x_obj) {
...
//	hwid=board_type_with_hw_id();//get_cci_hw_id();
	...
	APS_LOG("stk  obj->als_level_num  = %d , obj->als_value_num = %d \n",obj->als_level_num,obj->als_value_num);
...
static int stk3x1x_local_init(void)
{
	...
	APS_LOG("stk add i2c_add_driver(&stk3x1x_i2c_driver) ret = %d\n",ret);
	if (ret)
