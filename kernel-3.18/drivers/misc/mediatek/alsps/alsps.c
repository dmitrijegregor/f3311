...
static void als_work_func(struct work_struct *work)
{
	...
	printk("stk alsps after Light sensor lux value = %d, status = %d\n",value,status);
...
static void ps_work_func(struct work_struct *work)
{
	...
	if (NULL == cxt->ps_data.get_data) {
		printk("alsps driver not register data path\n");
		return;
	}
	...
	printk("stk alsps Proximity sensor ps value = %d, status = %d\n",value,status);
...
static int als_real_enable(int enable)
{
	...
	cxt = alsps_context_obj;
	printk("stk alsps enable= %d   cxt->is_als_active_data= %d cxt->is_als_active_nodata = %d \n", enable, cxt->is_als_active_data,cxt->is_als_active_nodata);
		...
		if (true == cxt->is_als_active_data || true == cxt->is_als_active_nodata) {
			...
			if (err)
				...
				if (err) {
					err = cxt->als_ctl.enable_nodata(1);
					if (err)
						printk("alsps enable(%d) err 3 timers = %d\n", enable, err);
				}
			}
			printk("alsps real enable\n");
	}

	if (0 == enable) {
		if (false == cxt->is_als_active_data && false == cxt->is_als_active_nodata) {
			printk("AAL status is %d\n", aal_use);
			if (aal_use == 0) {
				err = cxt->als_ctl.enable_nodata(0);
				if (err)
					printk("alsps enable(%d) err = %d\n", enable, err);

			}
			printk("alsps real disable\n");
		}
	}
	...
	for (i = 0; i < MAX_CHOOSE_ALSPS_NUM; i++) {
		...
		if (0 != alsps_init_list[i]) {
			...
			if (0 == err) {
				...
				printk("stk Nina alsps real driver %s probe fail err = %d \n", alsps_init_list[i]->name,err);
			}
		}
	}
...
int alsps_driver_add(struct alsps_init_info *obj)
{
	...
	ALSPS_FUN();
	if (!obj) {
		printk("ALSPS driver add fail, alsps_init_info is NULL\n");
		return -1;
	}

	for (i = 0; i < MAX_CHOOSE_ALSPS_NUM; i++) {
		if ((i == 0) && (NULL == alsps_init_list[0])) {
			printk("register alsps driver for the first time\n");
			if (platform_driver_register(&als_ps_driver))
				printk("failed to register alsps driver already exist\n");
		}
...
int ps_report_interrupt_data(int value)
{
	...
	/* int err =0; */
	cxt = alsps_context_obj;
	printk("stk Nina call ps_report_interrupt_data value = %d \n",value);
#ifdef ps_polling	
	if (cxt->is_get_valid_ps_data_after_enable == false) {
...
#endif	
	printk("stk Nina give ps value = %d\n",value);
	if (cxt->is_ps_batch_enable == false)
		{printk("stk Nina fail1\n");
		ps_data_report(cxt->idev, value, 3);
		printk("stk Nina fail2\n");
		}
	printk("stk Nina fail3\n");
	return 0;
}
/*----------------------------------------------------------------------------*/
...
int ps_register_control_path(struct ps_control_path *ctl)
{
	...
	if (!alsps_misc_dev_init) {
		...
		if (err) {
			...
			printk("stk create %s successfule\n",ALSPS_MISC_DEV_NAME);
		err = sysfs_create_group(&alsps_context_obj->mdev.this_device->kobj,
				&alsps_attribute_group);
		if (err < 0) {
			ALSPS_ERR("unable to create alsps attribute file\n");
			...
			printk("stk create alsps attribute filsuccessfule\n");
