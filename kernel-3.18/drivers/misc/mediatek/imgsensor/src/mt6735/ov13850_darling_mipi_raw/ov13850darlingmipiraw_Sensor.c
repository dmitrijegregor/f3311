...
#include "ov13850darlingmipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "OV13850darling_camera_sensor"
...
#define VIDEO_1080
#define GPIO_CAMERA_13M_ID_PIN  GPIO96
#define RES_9M
int ov13850_darling_chip_ver = OV13850_R2A;
//sensor otp
extern void otp_cali(unsigned char writeid);

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV13850_DARLING_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
{
#ifdef RES_9M
{4256, 3152, 76, 254, 4104, 2644, 4104, 2644, 4, 170, 4096, 2304, 0, 0, 4096, 2304}, // Preview
#else
...
#ifdef VIDEO_1080
 { 4256, 3152, 200, 492, 3856, 2168, 1928, 1084, 4, 2, 1920, 1080, 0, 0, 1920, 1080}, // video
#else
...
{
    return ((read_cmos_sensor(0x300A) << 8) | read_cmos_sensor(0x300B)) + 0x1;
}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (ov13850_darling_chip_ver == OV13850_R1A)
	{
	...
	else if(ov13850_darling_chip_ver == OV13850_R2A)
	{
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor(0x3801, 0x4C);
    write_cmos_sensor(0x3802, 0x00);// ;add for VGA differences
    write_cmos_sensor(0x3803, 0xfe);
    write_cmos_sensor(0x3804, 0x10);
    write_cmos_sensor(0x3805, 0x53);
    write_cmos_sensor(0x3806, 0x0b);// ;add for VGA differences
    write_cmos_sensor(0x3807, 0x51);
...
*/
int darling_capture_first_flag = 0;
int darling_pre_currefps = 0;
static void capture_setting(kal_uint16 currefps)
{
    LOG_INF("E! currefps:%d\n",currefps);

if (darling_pre_currefps != currefps)
{
	darling_capture_first_flag = 0;
	darling_pre_currefps = currefps;
}
else
{
	darling_capture_first_flag = 1;
}
if (darling_capture_first_flag == 0)
{
    write_cmos_sensor(0x0100, 0x00);//
...
static void normal_video_setting(kal_uint16 currefps)
{
    LOG_INF("E! currefps:%d\n",currefps);
		...
		write_cmos_sensor(0x372f, 0xa0);//0x90
		write_cmos_sensor(0x3800, 0x00);
		write_cmos_sensor(0x3801, 0xc8);//0x08
		write_cmos_sensor(0x3802, 0x01);
		write_cmos_sensor(0x3803, 0xec);
		write_cmos_sensor(0x3804, 0x0f);
		write_cmos_sensor(0x3805, 0xd7);//0x97
		write_cmos_sensor(0x3806, 0x0a);
		write_cmos_sensor(0x3807, 0x63);
		...
		write_cmos_sensor(0x3811, 0x04);//0x04
		write_cmos_sensor(0x3813, 0x02);
		write_cmos_sensor(0x3814, 0x31);
		write_cmos_sensor(0x3815, 0x31);
		write_cmos_sensor(0x3820, 0x01);//0x02
		...
		write_cmos_sensor(0x402f, 0x08);
		write_cmos_sensor(0x4501, 0x3c);
		write_cmos_sensor(0x4601, 0x77); //0x83
		write_cmos_sensor(0x4603, 0x01);
		write_cmos_sensor(0x4837, 0x19);
		write_cmos_sensor(0x5401, 0x61);
		write_cmos_sensor(0x5405, 0x40);
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                mt_set_gpio_mode(GPIO_CAMERA_13M_ID_PIN, GPIO_MODE_GPIO);
                mt_set_gpio_dir(GPIO_CAMERA_13M_ID_PIN, GPIO_DIR_IN);
                mt_set_gpio_pull_enable(GPIO_CAMERA_13M_ID_PIN, GPIO_PULL_DISABLE);
                CAM_13M_ID = mt_get_gpio_in(GPIO_CAMERA_13M_ID_PIN);
                if (CAM_13M_ID != 1)
                {
                    LOG_INF("ov13850 CAM_13M_ID = %d, it is ov13850 main source\n", CAM_13M_ID);
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                }
                LOG_INF("ov13850 CAM_13M_ID = %d\n", CAM_13M_ID);
                return ERROR_NONE;
            }
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id)
		...
		{
				LOG_INF("----R1A---- \n");
				ov13850_darling_chip_ver = OV13850_R1A;
		}else if((read_cmos_sensor(0x302a))==0xb2)
		{
				LOG_INF("----R2A---- \n");
				ov13850_darling_chip_ver = OV13850_R2A;
		}
...
UINT32 OV13850_DARLING_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    OV13850DARLING_MIPI_RAW_SensorInit    */
