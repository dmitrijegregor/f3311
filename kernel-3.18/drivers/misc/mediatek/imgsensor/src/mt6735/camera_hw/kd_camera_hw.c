...
struct pinctrl_state *cam0_ldo_avdd_en_h = NULL;
struct pinctrl_state *cam0_ldo_avdd_en_l = NULL;
struct pinctrl_state *cam1_ldo_avdd_en_h = NULL;
struct pinctrl_state *cam1_ldo_avdd_en_l = NULL;
...
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	/*externel LDO enable to set main camera AVDD enable pin*/
	cam0_ldo_avdd_en_h = pinctrl_lookup_state(camctrl, "cam0_ldo_avdd_en_1");
	if (IS_ERR(cam0_ldo_avdd_en_h)) {
		ret = PTR_ERR(cam0_ldo_avdd_en_h);
		pr_debug("%s : pinctrl err, cam0_ldo_avdd_en_h\n", __func__);
	}


	cam0_ldo_avdd_en_l = pinctrl_lookup_state(camctrl, "cam0_ldo_avdd_en_0");
	if (IS_ERR(cam0_ldo_avdd_en_l)) {
		ret = PTR_ERR(cam0_ldo_avdd_en_l);
		pr_debug("%s : pinctrl err, cam0_ldo_avdd_en_l\n", __func__);
	}
	/*externel LDO enable to set sub camera AVDD enable pin*/
	cam1_ldo_avdd_en_h = pinctrl_lookup_state(camctrl, "cam1_ldo_avdd_en_1");
	if (IS_ERR(cam1_ldo_avdd_en_h)) {
		ret = PTR_ERR(cam1_ldo_avdd_en_h);
		pr_debug("%s : pinctrl err, cam1_ldo_avdd_en_h\n", __func__);
	}


	cam1_ldo_avdd_en_l = pinctrl_lookup_state(camctrl, "cam1_ldo_avdd_en_0");
	if (IS_ERR(cam1_ldo_avdd_en_l)) {
		ret = PTR_ERR(cam1_ldo_avdd_en_l);
		pr_debug("%s : pinctrl err, cam1_ldo_avdd_en_l\n", __func__);
	}
	return ret;
}
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case CAMAVDD:
		if (PinIdx == 0) {
			if (Val == 0)
				pinctrl_select_state(camctrl, cam0_ldo_avdd_en_l);
			else
				pinctrl_select_state(camctrl, cam0_ldo_avdd_en_h);
		} else {
			if (Val == 0)
				pinctrl_select_state(camctrl, cam1_ldo_avdd_en_l);
			else
				pinctrl_select_state(camctrl, cam1_ldo_avdd_en_h);
		}
		break;
	default:
...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if ((currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV13850_MIPI_RAW) || 0 == strcmp(currSensorName, SENSOR_DRVNAME_OV13850_DARLING_MIPI_RAW))))
		{
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K5E2YA_MIPI_RAW, currSensorName) || 0 == strcmp(SENSOR_DRVNAME_S5K5E2YA_DARLING_MIPI_RAW, currSensorName))) {
			/* First Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
				PK_DBG("[PowerON] Get RST GPI11:%d\n", pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
			}
			/*Set AVDD_SUB_CAM_EN Low */
			mtkcam_gpio_set(pinSetIdx, CAMAVDD, GPIO_OUT_ZERO);
			PK_DBG("[PowerON] Get CAMAVDD GPIO20:%d\n", GPIO_OUT_ZERO);
			mdelay(1);
			...
			PK_DBG("[PowerON] Get PowerOn VCAM_IO: %d\n", VOL_1800);
			mdelay(1);

			//Enable VCAM_A
			mtkcam_gpio_set(pinSetIdx, CAMAVDD, GPIO_OUT_ONE);
			PK_DBG("[PowerON] Get VCAM_A GPIO20:%d\n", GPIO_OUT_ONE);
			mdelay(1);

			//Enable VCAM_D
			if (TRUE != _hwPowerOn(VRF18_1, VOL_1200)) {
				PK_DBG("[CAMERA SENSOR]Fail to enable digital power(VRF18_1),power id = %d\n", VRF18_1);
				goto _kdCISModulePowerOn_exit_;
			}
			PK_DBG("[PowerON] Get PowerOn VRF18_1_PMU: %d\n", VOL_1200);
			mdelay(1);

			//Enable XSHUTDN
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]);
				PK_DBG("[PowerON] Get RST GPIO11:%d\n", pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]);
			}
			mdelay(1);
		}
#if 0
		...
		if ((currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV13850_MIPI_RAW) || 0 == strcmp(currSensorName, SENSOR_DRVNAME_OV13850_DARLING_MIPI_RAW))))
		{
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
				PK_DBG("[PowerOFF] Disable RST GPIO11:%d\n", pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF]);
			}

			//Disable VCAM_D
			if (TRUE != _hwPowerDown(VRF18_1)) {
				PK_DBG("[CAMERA SENSOR]Fail to disable digital power(VRF18_1),power id = %d\n", VRF18_1);
				goto _kdCISModulePowerOn_exit_;
			}
			PK_DBG("[PowerON] Get PowerOff VRF18_1_PMU: %d\n", VOL_1200);
			mdelay(1);

			//Disable VCAM_A
			mtkcam_gpio_set(pinSetIdx, CAMAVDD, GPIO_OUT_ZERO);
			PK_DBG("[PowerOFF] Get VCAM_A GPIO20:%d\n", GPIO_OUT_ZERO);
			mdelay(1);
			...
			PK_DBG("[PowerOFF] Get PowerOff VCAMIO :%d\n", VOL_1800);
			mdelay(1);
		}
#if 0
		if ((currSensorName && (0 == strcmp(currSensorName, "imx135mipiraw"))) ||
