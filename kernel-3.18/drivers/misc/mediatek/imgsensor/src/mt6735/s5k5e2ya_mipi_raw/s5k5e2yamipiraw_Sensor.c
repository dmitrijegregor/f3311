...
#define S5K5E2YAOTP
kal_uint32 RG_Ratio_Typical = 0x332;//0x462;
kal_uint32 BG_Ratio_Typical = 0x2c7;//0x2bc;
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (CAM_5M_ID != 0)
				{
					LOG_INF("s5kse2ya CAM_5M_ID = %d, it is s5kse2ya second source\n", CAM_5M_ID);
...
static kal_uint32 apply_otp(struct otp_struct *otp_ptr)
{
	...
	if((*otp_ptr).flag == 0x01){
		...
		R_ration = (RG_Ratio_Typical*1000)/ RG_Ratio_Current;
		B_ration = (BG_Ratio_Typical*1000)/ BG_Ratio_Current;
