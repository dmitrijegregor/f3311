...
#include "s5k5e2yadarlingmipiraw_Sensor.h"

#include "../../../../cei_hw_id/cei_hw_id.h"

/****************************Modify following Strings for debug****************************/
#define PFX "s5k5e2yadarling_camera_sensor"
//set 1lane debug info
#define LOG_1 LOG_INF("s5k5e2ya,MIPI 1LANE\n")
#define LOG_2 LOG_INF("preview 1280*960@30fps,864Mbps/lane; video 1280*720@30fps,500Mbps/lane; capture 5M@17fps,864Mbps/lane\n")
//set 2lane debug info
#define LOG_3 LOG_INF("s5k5e2ya,MIPI 2LANE\n")
#define LOG_4 LOG_INF("preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n")
...
#define GPIO_CAMERA_5M_ID_PIN  GPIO94

static bool MIPI_1LANE = FALSE;
static imgsensor_info_struct imgsensor_info;
//Reducing PCLK for power consumption
#define REDUCE_PCLK


#define S5K5E2YADARLINGOTP
//CEI comments start

//kal_uint32 DARLING_RG_Ratio_Typical = 0x332;//0x462;
kal_uint32 DARLING_RG_Ratio_Typical = 0x2ad;//0x462;*
//kal_uint32 DARLING_BG_Ratio_Typical = 0x2c7;//0x2bc;
kal_uint32 DARLING_BG_Ratio_Typical = 0x24a;//0x2bc;*
//CEI comments end


#ifdef S5K5E2YADARLINGOTP
struct otp_struct{
    kal_uint32 flag;
    kal_uint32 vendor_code;
    kal_uint32 calibr_version;
    kal_uint32 rg_ratio;
    kal_uint32 bg_ratio;
    kal_uint32 checksum;
};
...
static imgsensor_info_struct imgsensor_info_1lane = {
	.sensor_id = S5K5E2YA_DARLING_SENSOR_ID,
...
static imgsensor_info_struct imgsensor_info_2lane = {
	.sensor_id = S5K5E2YA_DARLING_SENSOR_ID,
...
{
	return ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001)) + 0x1;
...
static void preview_setting(void)
{
	...
	if (MIPI_1LANE)
	write_cmos_sensor(0x0114,0x00);  //Lane mode 0:1lane, 1:2lane
	else
	...
	//Integration time
	if (MIPI_1LANE) {
	write_cmos_sensor(0x0202,0x07);  // coarse integration
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (MIPI_1LANE)
	write_cmos_sensor(0x0114,0x00);  //Lane mode
	else
	write_cmos_sensor(0x0114,0x01);  //Lane mode
	...
	if (MIPI_1LANE) {
	write_cmos_sensor(0x0340,0x07); // frame_length_lines : def. 1962d (7C2 --> 7A6 Mimnimum 22 lines)
	write_cmos_sensor(0x0341,0x97);
	write_cmos_sensor(0x0342,0x14); // line_length_pck : def. 2900d
...
// Reset for operation
	...
	if (!MIPI_1LANE) {
		...
		write_cmos_sensor(0x0340,0x06); // frame_length_lines : def. 1962d (7C2 --> 7A6 Mimnimum 22 lines)
		...
		write_cmos_sensor(0x0202,0x02);  // coarse integration
		write_cmos_sensor(0x0203,0x00);
		write_cmos_sensor(0x0200,0x04);
		write_cmos_sensor(0x0201,0x98);

		write_cmos_sensor(0x0204,0x00);  // fine integration (AA8h --> AC4h)
		write_cmos_sensor(0x0205,0x20);
	}
#else
	if (MIPI_1LANE)
	...
	// Size Setting
	if (MIPI_1LANE) {
	write_cmos_sensor(0x0340,0x05); // frame_length_lines : def. 1962d (7C2 --> 7A6 Mimnimum 22 lines)
	write_cmos_sensor(0x0341,0xB7);
	write_cmos_sensor(0x0342,0x14); // line_length_pck : def. 2900d
	write_cmos_sensor(0x0343,0xB0);
	} else {
	write_cmos_sensor(0x0340,0x07); // frame_length_lines : def. 1962d (7C2 --> 7A6 Mimnimum 22 lines)
...
// Reset for operation
	...
	write_cmos_sensor(0x0342,0x0F); // line_length_pck : def. 2900d
	write_cmos_sensor(0x0343,0x19);
	} else {
	write_cmos_sensor(0x0340,0x07); // frame_length_lines : def. 1962d (7C2 --> 7A6 Mimnimum 22 lines)
	write_cmos_sensor(0x0341,0xD0);
	write_cmos_sensor(0x0342,0x0B); // line_length_pck : def. 2900d 
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				mt_set_gpio_mode(GPIO_CAMERA_5M_ID_PIN, GPIO_MODE_GPIO);
				mt_set_gpio_dir(GPIO_CAMERA_5M_ID_PIN, GPIO_DIR_IN);
				mt_set_gpio_pull_enable(GPIO_CAMERA_5M_ID_PIN, GPIO_PULL_DISABLE);
				CAM_5M_ID = mt_get_gpio_in(GPIO_CAMERA_5M_ID_PIN);
				if (CAM_5M_ID != 1)
				{
					LOG_INF("s5kse2ya CAM_5M_ID = %d, it is s5kse2ya main source\n", CAM_5M_ID);
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				LOG_INF("s5kse2ya CAM_5M_ID = %d\n", CAM_5M_ID);
				return ERROR_NONE;
			}
...
#ifdef S5K5E2YADARLINGOTP
// return value:
// err_read: 1 no otp info, 0 valid otp info

static kal_uint32 read_otp(struct otp_struct *otp_ptr)
{

	kal_uint32 otp_flag=0, addr=0;
	kal_uint32 otp_page=2, otp_group=1, err_read=0;
	...
	if(otp_flag == 0x01){
		addr = 0x0A04;
		LOG_INF("[CY33] Page2 Group1--OTP base address is 0x%x, Value is 0x%x\n", addr, otp_flag);
	}
	else if(otp_flag == 0x00){
		addr = 0;
 		otp_group = 2;
		LOG_INF("[CY33] Page2 Group1--OTP data is empty\n");
	}else if((otp_flag == 0x10) || (otp_flag == 0x11)){
		addr = 0;
		otp_group = 2;
		LOG_INF("[CY33] Page2 Group1--OTP data is invalid\n");
	}

	if(otp_group == 2){
		otp_flag = read_cmos_sensor(0x0A24);
		if(otp_flag == 0x01){
			addr = 0x0A24;
			LOG_INF("[CY33] Page2 Group2--OTP base address is 0x%x, Value is 0x%x\n", addr, otp_flag);
		}
		else if(otp_flag == 0x00){
			addr = 0;
			otp_page = 3;
			LOG_INF("[CY33] Page2 Group2--OTP data is empty\n");
		}
		else if((otp_flag == 0x10) || (otp_flag == 0x11)){
			addr = 0;
			otp_page = 3;
			LOG_INF("[CY33]Page2 Group2--OTP data is invalid\n");
		}
	}
	...
	if(otp_page == 3){
		...
		if(otp_flag == 0x01){
			addr = 0x0A04;
			LOG_INF("[CY33] Page3 Group3--OTP base address is 0x%x, Value is 0x%x\n", addr, otp_flag);
		}
		else if(otp_flag == 0x00){
			addr = 0;
			LOG_INF("[CY33] Page3 Group3--OTP data is empty\n");
		}
		else if((otp_flag == 0x10) || (otp_flag == 0x11)){
			addr = 0;
			LOG_INF("[CY33] Page3 Group3--OTP data is invalid\n");
		}
	}
	...
	if(addr != 0){
		err_read=0;
		(*otp_ptr).flag = 0x01;
		(*otp_ptr).vendor_code = read_cmos_sensor(addr+1);
		(*otp_ptr).calibr_version = read_cmos_sensor(addr+2);
		(*otp_ptr).rg_ratio = ((read_cmos_sensor(addr+10)<<8) + (read_cmos_sensor(addr+11)));
		(*otp_ptr).bg_ratio = ((read_cmos_sensor(addr+12)<<8) + (read_cmos_sensor(addr+13)));

		LOG_INF("[CY33]Read OTP sensor flag: Address is 0x%x, Value is 0x%x\n", addr, (*otp_ptr).flag);
		LOG_INF("[CY33]Read OTP sensor vendor_code: Address is 0x%x, Value is 0x%x\n",  addr+1, (*otp_ptr).vendor_code);
		LOG_INF("[CY33]Read OTP sensor calibr_version: Address is 0x%x, Value is 0x%x\n",  addr+2, (*otp_ptr).calibr_version);
		LOG_INF("[CY33]Read OTP sensor rg_ratio: Address is 0x%x, Value is 0x%x\n", addr+10, (*otp_ptr).rg_ratio);
		LOG_INF("[CY33]Read OTP sensor bg_ratio: Address is 0x%x, Value is 0x%x\n", addr+12, (*otp_ptr).bg_ratio);
	}
	else{
		err_read=1;
		(*otp_ptr).flag = 0;
		(*otp_ptr).vendor_code = 0;
		(*otp_ptr).calibr_version = 0;
		(*otp_ptr).rg_ratio = 0;
		(*otp_ptr).bg_ratio = 0;
	}
...
// return value:
// err_apply: 1 no otp info, 0 valid otp info
static kal_uint32 apply_otp(struct otp_struct *otp_ptr)
{
	kal_uint32 gain_default=0x100;
	kal_uint32 RG_Ratio_Current, BG_Ratio_Current;
	kal_uint32 R_ration, B_ration, R_gain, G_gain, B_gain, Gr_gain, Gb_gain,err_apply;

	if((*otp_ptr).flag == 0x01){
		err_apply =0;
		RG_Ratio_Current = (*otp_ptr).rg_ratio;
		BG_Ratio_Current = (*otp_ptr).bg_ratio;
		R_ration = (DARLING_RG_Ratio_Typical*1000)/ RG_Ratio_Current;
		B_ration = (DARLING_BG_Ratio_Typical*1000)/ BG_Ratio_Current;

		if(R_ration>=1000) 
		{
			if(B_ration>=1000)  //R_ration>=1000 & B_ration>=1000
			{
				G_gain=gain_default;
				R_gain=(gain_default*R_ration)/1000;
				B_gain=(gain_default*B_ration)/1000;
			}
			else		 //R_ration>=1000 & B_ratio<1000
			{
				B_gain=gain_default;
		...
		else
		{
			...
			else if ((R_ration>=1000)&&(B_ration<=1000))	//R_ration<1000 & (R_ration>=1000)&&(B_ration<=1000)
			{
				B_gain=gain_default;
				G_gain=(gain_default*1000)/B_ration;
				R_gain=(gain_default*R_ration)/B_ration;
			}
			else			//R_ration<1000 & B_ration>1000
			{
				R_gain=gain_default;
				G_gain=(gain_default*1000)/R_ration;
				B_gain=(gain_default*B_ration)/R_ration;
		...
		Gr_gain=G_gain;   // Due to GrGb Ratio is 0x400(1024), so that Gr=Gb=G gain
		Gb_gain=G_gain;

		if(R_gain > 0x100){
			write_cmos_sensor(0x210, R_gain >>8);
			write_cmos_sensor(0x211, R_gain & 0x00ff);
		}
		if(Gr_gain > 0x100){
			write_cmos_sensor(0x20E, Gr_gain >>8);
			write_cmos_sensor(0x20F, Gr_gain & 0x00ff);
		}
		if(Gb_gain > 0x100){
			write_cmos_sensor(0x214, Gb_gain >>8);
			write_cmos_sensor(0x215, Gb_gain & 0x00ff);
		}
		if(B_gain > 0x100){
			write_cmos_sensor(0x212, B_gain >>8);
			write_cmos_sensor(0x213, B_gain & 0x00ff);
		}
	}else{
		err_apply=1;
	}
...
#ifdef S5K5E2YADARLINGOTP
	kal_uint8 err_open_read, err_open_apply;
	struct otp_struct otp_ptr;
	LOG_INF("[CY33] PLATFORM:MT6735\n");
#endif
...
#ifdef S5K5E2YADARLINGOTP
	mdelay(10);
	err_open_read=read_otp(&otp_ptr);
	if(err_open_read==1){
		LOG_INF("[CY33] Read OTP Fail\n");
	}

	err_open_apply=apply_otp(&otp_ptr);
	if(err_open_apply==1){
		LOG_INF("[CY33] Apply OTP Fail\n");
	}
#endif
...
UINT32 S5K5E2YA_DARLING_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	int hw_type = 0;
	...
	if (hw_type < CEI_HW_DVT1) {
		MIPI_1LANE = TRUE;
		imgsensor_info = imgsensor_info_1lane;
	} else {
		MIPI_1LANE = FALSE;
		imgsensor_info = imgsensor_info_2lane;
	}

	return ERROR_NONE;
}	/*	S5K5E2YA_DARLING_MIPI_RAW_SensorInit	*/
