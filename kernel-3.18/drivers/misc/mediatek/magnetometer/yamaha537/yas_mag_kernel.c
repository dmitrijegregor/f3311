...
static int yas_device_read(int32_t type, uint8_t addr, uint8_t *buf, int len)
{
	...
	//Printhh("[%s] enter..ok\n", __FUNCTION__);
	//Printhh("[%s] this_client->addr = 0x%2X  timing is %d, len is %d\n", __FUNCTION__, this_client->addr, this_client->timing, len);  
...
//#if 0
	...
	//Printhh("[%s] enter..ok03\n", __FUNCTION__);  
	}

//#else
	else
	{
	//Printhh("[%s] enter..ok03_01\n", __FUNCTION__);  
	#if 0
		if(unlikely(NULL == st->dma_va))
		{
			this_client->ext_flag &= I2C_MASK_FLAG; //CLEAR DMA FLAG	
	...
	#endif
		{
			//Printhh("[%s] enter..ok04\n", __FUNCTION__);  
			memset(st->dma_va, 0, 1024);
			msg[1].buf = (char *)(st->dma_pa);
			//Printhh("[%s] enter..ok05\n", __FUNCTION__);  
			err = i2c_transfer(this_client->adapter, (struct i2c_msg *)msg, 2);
			if (err != 2) {
				dev_err(&this_client->dev,
						"i2c_transfer() read error: "
			...
			//Printhh("[%s] enter..ok06\n", __FUNCTION__);  
			memcpy(buf, st->dma_va, len);
			//Printhh("[%s] enter..ok07\n", __FUNCTION__);  
		}
	}
//#endif
//	MSE_LOG("%s   successful\n", __FUNCTION__);
	//Printhh("[%s] enter..ok08\n", __FUNCTION__);  
	mutex_unlock(&yas537_i2c_mutex);
	return 0;
}
...
#define GPIO_GMC303_GP61 61  //jonny gp61
...
//jonny cy33 s
	Printhh("[%s] this_client->addr S= 0x%2X\n", __FUNCTION__, this_client->addr);
	//this_client->addr = 0x2E;
	Printhh("[%s] this_client->addr E= 0x%2X\n", __FUNCTION__, this_client->addr);
//jonny cy33 e
#endif
...
/********try to alloc dma memory 3times************/
	this_client->dev.coherent_dma_mask = DMA_BIT_MASK(32); //jonny cy33
...
//mdelay(g_iDelay);
...
#if 1   //henry tt current
	#if 0	// VY36 , Grace modify for cy33
	...
	#else	
	if( (g_iHwId == 0x0) || (g_iHwId == 0x01) )
	{
		//PDP1:0x0, PDP2:0x1
		Printhh("[%s] hwid=%d , set GPIO input_high.. \n", __FUNCTION__, g_iHwId);
		mt_set_gpio_dir(GPIO_GMC303_GP61, 0);
		mt_set_gpio_out(GPIO_GMC303_GP61, 1);
		udelay(100);
	}
	else{
		//SP:0x2, AP:0x3, TP:0x4, PQ:0x5, or others
		// do nothing
		Printhh("[%s] hwid=%d , GPIO do nothing.. \n", __FUNCTION__, g_iHwId);
	}
	#endif
...
static const struct of_device_id mag_of_match[] = {
	...
	{.compatible = "mediatek,msensor_second"}, //jonny cy33
	{},
};
#endif
#endif
