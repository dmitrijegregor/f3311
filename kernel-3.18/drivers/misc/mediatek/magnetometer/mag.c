...
static struct platform_device *pltfm_dev; //jonny m
...
//jonny m s
struct platform_device *get_gmcms_platformdev(void)
{
	return pltfm_dev;
}
//jonny m e
...
#if 1
	if (dev != NULL) {
		MAG_ERR("mag_input_destroy not NULL!!\n");
		input_unregister_device(dev);
		input_free_device(dev);
	}else {
		MAG_ERR("mag_input_destroy is  NULL!!\n");
	}
#endif
}
*/
