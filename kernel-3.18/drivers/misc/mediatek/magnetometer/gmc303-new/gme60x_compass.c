...
//UART suspend config S
#if 1
#define NUM_OF_UART_GPIO 3
struct uart_gpio {
	int gpio;	
}; 
static struct uart_gpio cei_uart_gpios[] = {
	{74},
	{75},
	{87},
};
#endif
//UART suspend config E
...
#if GPIO61_LOW
static struct platform_device *alspsPltFmDev= NULL;  //jonny gp61 
#endif
...
#if GPIO61_LOW
	{.compatible = "mediatek,gmc303_gp"}, //jonny gp61 
#endif
	{},
};
#endif
...
static int gme60x_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	int i = 0; //UART suspend config 
//UART suspend config S
#if 1
	for (i = 0; i < NUM_OF_UART_GPIO; i++) {
		mt_set_gpio_mode(cei_uart_gpios[i].gpio, 0);
		mt_set_gpio_dir(cei_uart_gpios[i].gpio, GPIO_DIR_IN);
		mt_set_gpio_pull_enable(cei_uart_gpios[i].gpio, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(cei_uart_gpios[i].gpio, GPIO_PULL_DOWN);
		//gpio_request(cei_uart_gpios[i].gpio, cei_uart_gpios[i].gpio_name);
	}
#endif
//UART suspend config E
...
//jonny gp61 s
#if GPIO61_LOW
static int gmc303_setup_gpio(struct i2c_client *client)
{	
	int err = -1;
	struct gme60x_i2c_data *ps_data = i2c_get_clientdata(client);

	alspsPltFmDev = get_gmcms_platformdev();
...
while (1) {
			...
			MAG_ERR("GME60X gme60x_probe done %d\n",err);
			//goto exit_init_failed;
			}
		}
#endif
...
//jonny  gp61 s
	...
	udelay(100);
#endif
	//state = gpio_get_value(GPIO_GMC303_GP61);
	//MAGN_LOG("[%s] GPIO 61 is = %d\n", __func__, state);
//jonny gp61 e
