...
/*---------------------  Static Classes  ----------------------------*/
int g_iDelayStk = 500;
// S- Add for gsensor read miscTA
int g_iMiscTaXyz[4] = {-1, -1, -1, -1};
static int g_first_store_nv_cali = 1;		// 0=false , 1=true
// E- Add for gsensor read miscTA
...
//#define IRQ_GPIO_NUM 13	//VY36
#define IRQ_GPIO_NUM 67 
...
// 20160412 Grace sync VY36
	...
	else
	{
		...
		if(res <= 0)
		{
			GSE_LOG("%s: i2c_master_send failed!\n", __FUNCTION__);
			err = STK8313_ERR_I2C;
			goto i2c_err_exit;
		}
		client->addr = client->addr& I2C_MASK_FLAG;
...
/*----------------------------------------------------------------------------*/
// STK8313 no need to check device id , the 0x0E register is "Reserved"
#if 0
...
static int STK_i2c_Rx(char *rxData, int length)
{
	...
	//hh: protect client->addr
	mutex_lock(&stk8313_i2c_mutex);
	res = stk831x_hwmsen_read_block(client, addr, rxData, length);
	mutex_unlock(&stk8313_i2c_mutex);
	...
	bool enable)
{
	...
	if (rc)
		goto err;
	rc = write_retry(obj->client, STK8313_REG_MODE, (0|INT_PUSH_PULL) );
	if (rc)
		goto err;
	...
	cfg.range = (cfg.range & ~SHAKE_THRESH_M) | g_iSTH;
	...
	if (rc)
		goto err;
	rc = write_retry(obj->client, STK8313_REG_RANGE, cfg.range);
	if (rc)
		goto err;
err:
	if (rc)
	...
	enum acc_client request)
{
	...
	if (req != stk8313_op_mode.req) {
		...
		if (enable) {
			...
			u8 mode = MODE_ACTIVE | INT_PUSH_PULL;
	...
	enum acc_client request)
{
	...
	Printhh("[%s] enter.. enable=%d, request=%d\n", __FUNCTION__, enable, request);
...
static ssize_t store_cali_miscTa_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	// S- Add for gsensor read miscTA , do nothing before nvram_daemon while booting
	if (g_first_store_nv_cali)
	{
		Printhh("[%s] g_first_store_nv_cali=%d , do nothing before nvram_daemon\n", __FUNCTION__, g_first_store_nv_cali );
	}
	else
	{
	...
	// E- Add for gsensor read miscTA , do nothing before nvram_daemon while booting
...
static ssize_t stk831x_store_send(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	databuf[0] = addr;
	databuf[1] = dat;
	GSE_LOG("send(%02X, %02X) = %d\n", addr, cmd, hwmsen_write_byte(obj->client, addr, cmd) );
...
static struct driver_attribute *stk8313_attr_list[] = {
	...
	&driver_attr_status,        
	&driver_attr_cali_miscTa,		/*Add for cali data in miscTA*/
	&driver_attr_tilt,
	&driver_attr_pmrelax,
	&driver_attr_reg,
	&driver_attr_sth,
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_SET_CALI:
			...
			{   
				...
				if (g_first_store_nv_cali)
				{
					g_first_store_nv_cali = 0;		// 0=false , 1=true
				}
				// E- Add for gsensor read miscTA
...
//henry: not enter here.
	...
	// S- Add for gsensor read miscTA
	g_first_store_nv_cali = 1;		// 0=false , 1=true
	// E- Add for gsensor read miscTA
