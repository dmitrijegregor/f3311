			    ...
			    unsigned long *delay_on, unsigned long *delay_off)
{
	if (mt_mt65xx_blink_set(led_cdev, delay_on, delay_off))
		return 0; // Mike, force using MTK default blinking hardware only.
	else
		return 0;
}
...
#endif
/* show/store prototype.
 * ssize_t (*show)(struct device *dev, struct device_attribute *attr, char *buf);
 * ssize_t (*store)(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
 * #define DEVICE_ATTR_RW(name) struct device_attribute dev_attr_##_name
 */
...
static ssize_t delay_off_show(struct device *, struct device_attribute *, char *);
static ssize_t delay_off_store(struct device *, struct device_attribute *, const char *, size_t );
static DEVICE_ATTR_RW(delay_off);

static ssize_t blink_brightness_show(struct device *, struct device_attribute *, char *);
static ssize_t blink_brightness_store(struct device *, struct device_attribute *, const char *, size_t );
static DEVICE_ATTR_RW(blink_brightness);

static ssize_t blink_show(struct device *, struct device_attribute *, char *);
static ssize_t blink_store(struct device *, struct device_attribute *, const char *, size_t );
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		g_leds_data[i]->cdev.blink_set = mt65xx_blink_set; // Mike, will call to mt_mt65xx_blink_set().
		...
		// Mike, for extend interface (blinking)
		do {
	...
	// Mike, disable CHG_LED_CTL, LEDR_CTL, LEDB_CTL, LEDG_CTL pins.
	do {
		LEDS_DRV_DEBUG("Disable CHG_CTL pin\n");
		mt_set_gpio_mode(93,0); mt_set_gpio_dir(93, GPIO_DIR_OUT); mt_set_gpio_out(93, GPIO_OUT_ONE);
		mt_set_gpio_mode(83,0); mt_set_gpio_dir(83, GPIO_DIR_OUT);
		mt_set_gpio_mode(82,0); mt_set_gpio_dir(82, GPIO_DIR_OUT);
		mt_set_gpio_mode(128,0); mt_set_gpio_dir(128, GPIO_DIR_OUT);
		switch( get_cei_hw_id() ) {
			case CEI_HW_DVT1:
			case CEI_HW_EVT1:
			case CEI_HW_EVT2:
				mt_set_gpio_out(83, GPIO_OUT_ONE); mt_set_gpio_out(82, GPIO_OUT_ONE); mt_set_gpio_out(128, GPIO_OUT_ONE);
				break;
			case CEI_HW_DVT2:
			default:
				mt_set_gpio_out(83, GPIO_OUT_ZERO); mt_set_gpio_out(82, GPIO_OUT_ZERO); mt_set_gpio_out(128, GPIO_OUT_ZERO);
		}
	} while(0);
...
static ssize_t blink_brightness_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size) {
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	unsigned long state;
	ssize_t ret = -EINVAL;
...
static ssize_t blink_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size) {
	...
	if (!(led_cdev->flags & LED_SUSPENDED)) {
		// Mike ignore ERALY_SUSPEND here.
