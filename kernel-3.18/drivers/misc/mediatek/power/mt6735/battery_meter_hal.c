...
//CEI comments S
	if (g_fg_is_charging == KAL_TRUE) {
		bm_print(BM_LOG_CRTI, "[fgauge_read_current] CEI(K) compensated current(charging) = %d\r\n",
			 dvalue*(-1));
	} else {
		bm_print(BM_LOG_CRTI, "[fgauge_read_current] CEI(K) compensated current(discharging) = %d\r\n",
			 dvalue);
	}
//CEI comments E
