...
static int mtk_regulator_set_voltage_sel(struct regulator_dev *rdev, unsigned selector)
{
  ...
  if(strcmp(rdesc->name,"VRF18_1")==0) {
		...
		{
		    PMICLOG("[May] pmic_config_interface(0xA86,0x1,0x1,2)\n");
...
void ldo_int_handler(void)
{
	...
	PMICLOG("[ldo_int_handler] Reg[0x%x]=0x%x\n", MT6328_INT_STATUS1, upmu_get_reg_value(MT6328_INT_STATUS1));
	PMICLOG("PMIC SD OC PMIC_OC_STATUS_VMCH =0x%x\n", upmu_get_reg_value(0x216));
	/* first check if ldo oc occur */
	if (1 == pmic_get_register_value(PMIC_RG_INT_STATUS_LDO_OC)) {
		/* second check if vmc or vmch oc occur */
		if (1 == pmic_get_register_value(PMIC_OC_STATUS_VMCH)) {
			msdc_sd_power_off();
			ret = pmic_set_register_value(PMIC_OC_STATUS_VMCH, 1); /* write 1 clear */
		} else if (1 == pmic_get_register_value(PMIC_OC_STATUS_VMC)) {
			msdc_sd_power_off();
			ret = pmic_set_register_value(PMIC_OC_STATUS_VMC, 1); /* write 1 clear */
		}
			pmic_set_register_value(PMIC_RG_INT_STATUS_LDO_OC, 1); /* write 1 clear */
	}
}
...
#define PLUG_NPLUG_INTERVAL 20
#define PLUG_NPLUG_COUNT 5
kal_bool bNSC = KAL_FALSE;
unsigned long UnplugTime = 0;
unsigned long LastUnplugTime = 0;
unsigned long UnplugCount = 0;
...
//[Feature] 22871 Invalid and weak charger pop up indication S
		UnplugTime = rtc_read_hw_time();
		if (LastUnplugTime) {
			if (abs(UnplugTime - LastUnplugTime) < PLUG_NPLUG_INTERVAL)
				UnplugCount ++;
			else {
				UnplugCount = 0;
				UnplugTime = 0;
		...
		LastUnplugTime = UnplugTime;
	 } else {
		if (UnplugCount > PLUG_NPLUG_COUNT)
		{
			bNSC = KAL_TRUE;
			UnplugTime = 0;
			LastUnplugTime = 0;
	 ...
	 pr_info("[chrdet_int_handler] UnplugTime = %lu, UnplugCount = %lu, bNSC = %d\n", UnplugTime, UnplugCount, bNSC);
//[Feature] 22871 Invalid and weak charger pop up indication E
...
void PMIC_EINT_SETTING(void)
{
	...
	pmic_register_interrupt_callback(25, ldo_int_handler);	/* sd oc */
	pmic_set_register_value(PMIC_RG_VMC_OCFB_EN, 1); /* sd oc */
	pmic_set_register_value(PMIC_RG_VMCH_OCFB_EN, 1); /* sd oc */
...
#endif
	pmic_enable_interrupt(25, 1, "PMIC");/* sd oc */
