...
//E [VY36 M] JackBB 2015/10/12 Kernel standardization
		...
		mutex_lock(&pn547_dev->i2c_mutex);//[VY3X] JackBB 2015/11/25 Add I2C mutex
#if NFC_DEBUG
		dev_info(&pn547_dev->client->dev, "pn547 : I2C Read Start\n");
#endif
		#ifdef Enable_I2C_DMA
		...
		#endif
#if NFC_DEBUG
		dev_info(&pn547_dev->client->dev, "pn547 : I2C Read End\n");
#endif
		mutex_unlock(&pn547_dev->i2c_mutex);//[VY3X] JackBB 2015/11/25 Add I2C mutex
...
//E [VY36 M] JackBB 2015/10/12 Kernel standardization
		...
		mutex_lock(&pn547_dev->i2c_mutex);//[VY3X] JackBB 2015/11/25 Add I2C mutex
#if NFC_DEBUG
		dev_info(&pn547_dev->client->dev, "pn547 : I2C write start\n");
#endif
		/* Write data */
		do {
				retry--;
...
#if NFC_DEBUG
		dev_info(&pn547_dev->client->dev, "pn547 : I2C write end\n");
#endif
		mutex_unlock(&pn547_dev->i2c_mutex);//[VY3X] JackBB 2015/11/25 Add I2C mutex
...
static int pn547_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = mt_nfc_pinctrl_select(gpctrl, st_eint_l);
		...
		mutex_init(&pn547_dev->i2c_mutex);
    ...
    I2CDMAReadBuf = (char *)dma_alloc_coherent(&client->dev, MAX_BUFFER_SIZE, (dma_addr_t *)&I2CDMAReadBuf_pa, GFP_KERNEL);
		...
		if (node) {
			...
			dev_info(&client->dev, "%s : nfc_irq = %d\n", __func__, nfc_irq);
		...
		*/
		...
		err_misc_register:
				...
				mutex_destroy(&pn547_dev->i2c_mutex);
				kfree(pn547_dev);
		err_exit:
