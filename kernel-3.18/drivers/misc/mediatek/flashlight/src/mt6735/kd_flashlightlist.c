...
static ssize_t flashlight_control_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", (unsigned int)g_flash_ctrl);
}

static ssize_t flashlight_control_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	ssize_t ret = -EINVAL;
	unsigned long control = 0;

	ret = kstrtoul(buf, 10, &control);

	if (ret)
		return ret;

	g_flash_ctrl = control;

	return size;
}
static DEVICE_ATTR(control, 0644, flashlight_control_show, flashlight_control_store);
...
#define FLASHLIGHT_MAJOR 242
	...
	ret = device_create_file(flashlight_device, &dev_attr_control);
	if (ret) {
		printk(KERN_ERR "[Flashlight]device_create_file control failed\n");
		goto flashlight_probe_error;
	}
	/* initialize members */
