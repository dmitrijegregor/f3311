...
// BSP_Audio_S: 20160112 FP019378:Non-CTIA headset warning message
static ssize_t switch_dev_print_name(struct switch_dev *swdev, char *buf);
// BSP_Audio_E: 20160112


static struct hrtimer timer_set_audio_register_20s;
#define DELAY_TIME_20S 20

static enum hrtimer_restart set_audio_register_func(struct hrtimer *timer)
{

	int current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0) >> 6);
	int current_micbias = pmic_pwrap_read(ACCDET_ADC_REG);
	ACCDET_DEBUG("[Accdet] current_status = %d, current_micbias = %d\n",current_status,current_micbias);
	if(accdet_status == PLUG_OUT && eint_accdet_sync_flag ==1 && current_status ==3 && current_micbias ==0)
	{
		ACCDET_DEBUG("[Accdet]Set Reg 0x0d00 value =700\n");
		pmic_pwrap_write(ACCDET_ADC_REG, 700);
	}
	return HRTIMER_NORESTART;
}
// BSP_Audio_E: 2016_0412
...
char *accdet_report_string[4] = {
	...
	"Headset_mic",
	"Headset_no_mic",
	"HEADSET_illegal",  // BSP_Audio: 20160112 FP019378:Non-CTIA headset warning message
	/* "Double_check"*/
};
/****************************************************************/
/***        export function                                                                        **/
	...
	case PLUG_OUT:
		...
		if (current_status == 0) {
			...
			if (current_status == 0 && show_icon_delay != 0) {
				...
				/*accdet_auxadc_switch(0);*/
				if (520 > pin_adc_value && pin_adc_value > 90) {	/*90mv   ilegal headset*/
...
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
	pmic_pwrap_write(ACCDET_DEBOUNCE0, cust_headset_settings->debounce0);
/* BSP_Audio_S: FP019378:Non-CTIA headset warning message */
#if 0
	pmic_pwrap_write(ACCDET_DEBOUNCE1, 0xFFFF);	/*2.0s*/
#else
	pmic_pwrap_write(ACCDET_DEBOUNCE1, 0x7FFF); // change to 1.0s
#endif
/* BSP_Audio_E: FP019378:Non-CTIA headset warning message */
	pmic_pwrap_write(ACCDET_DEBOUNCE3, cust_headset_settings->debounce3);
   ...
   /**************************************AUXADC enable auto sample****************************************/
	pmic_pwrap_write(ACCDET_AUXADC_AUTO_SPL, (pmic_pwrap_read(ACCDET_AUXADC_AUTO_SPL) | ACCDET_AUXADC_AUTO_SET));
       ...
       ACCDET_DEBUG("[Accdet] timer_set_audio_register_20s start\n");
       hrtimer_start(&timer_set_audio_register_20s,ktime_set(DELAY_TIME_20S, 0),HRTIMER_MODE_REL);
	// BSP_Audio_E: 2016_0412
}
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	accdet_data.print_name = switch_dev_print_name;  // BSP_Audio_S: 20160112 FP019378:Non-CTIA headset warning message
	...
	wake_lock_init(&accdet_timer_lock, WAKE_LOCK_SUSPEND, "accdet timer wakelock");
       ...
       ACCDET_DEBUG("[Accdet]timer_set_audio_register_20s init\n");
	hrtimer_init(&timer_set_audio_register_20s, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
       timer_set_audio_register_20s.function = set_audio_register_func;
       //BSP_Audio_E: 2016_0412
#if DEBUG_THREAD
