...
//#define CONFIG_TOUCHSCREEN_HIMAX_DEBUG  20151215 himax_852xES.h defind
...
static int himax_tpd_int_gpio = 10;//20160330 int gpio 10
...
void himax_rst_gpio_set(int pinnum, uint8_t value)
{
	...
	retval = gpio_request_one(pinnum, GPIOF_OUT_INIT_LOW,
				 "touchp_reset");
	if (retval < 0) {
		I("Unable to request gpio reset_pin\n");
	}*/
...
int himax_gpio_power_config(struct i2c_client *client,struct himax_i2c_platform_data *pdata)//20160517
{
	//int error=0;
	int retval = 0;
	private_pdata=pdata;//20160517 add
	atomic_set(&private_pdata->int_state,1);//20160521 int_state default value
/*	
#ifdef CONFIG_OF_TOUCH
...
*/
	...
	retval = gpio_request_one(pdata->gpio_irq, GPIOF_IN,
				 "tpd_int");
	if (retval < 0) {
		I("Unable to request gpio int_pin\n");
		gpio_free(pdata->gpio_irq);
	}
