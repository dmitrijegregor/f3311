...
#include "../../../../watchdog/mediatek/wdt/mt6735/mt_wdt.h" //20160119 reboot every 30s issue
#include <mach/wd_api.h> //20160119 reboot every 30s issue
//============== 20160517
#include <linux/irq.h>
...
static u8 g_proximity_en = 0;
	...
	{
		#include "CEI_CY33_E1904_CFG06_20160624_TRULY.i"
		
		
		
		
	};
	//===================================================== 2nd source DJN 20160201

	static unsigned char i_DJN_FW[]=
	{
		
		#include "CEI_CY33_E1704_CFG22_20160420_DJN.i" //version 0x1704,cfg 0x22
	};
	//=====================================================
#endif

#if CHECK_CTP_ID //20160201 TPID_CHECK
...
//20160419 add Touch panel Recovery
#ifdef HX_TOUCH_REC
...
static int FW_doupdate_M=0; //20160302 fw_version update Truly
static int FW_doupdate_S=0; //20160302 fw_version update DJN
...
static struct of_device_id himax_match_table[] = {
	{.compatible = "mediatek,cap_touch_himax" },	
	{},
};
...
static int of_get_himax85xx_platform_data(struct device *dev)
{
	...
	//I("g_vproc_en_gpio_number %d\n", tpd_rst_gpio_number);
	//I("g_vproc_vsel_gpio_number %d\n", tpd_int_gpio_number);
	return 0;
}
...
static int himax_input_register(struct himax_ts_data *ts)
{
	...
	/* 20160225 mask for zoomin zommout
	...
	if(IC_TYPE == HX_85XX_ES_SERIES_PWON)
		{
			...
			HX_TX_NUM = 19;
			//===20160301
			//HX_MAX_PT = (data[2] & 0xF0) >> 4; // FE(72) 20160301
			HX_MAX_PT = 5;
#ifdef HX_EN_SEL_BUTTON
			//HX_BT_NUM = (data[2] & 0x0F); //FE(72) 20160301
			HX_BT_NUM = 0;
#endif
			/* 20160301
...
static int i_update_FW(void)
{
	...
	if (( private_ts->vendor_fw_ver_H < i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR] )
		...
		{
			fw_status = 2; //20160119 update fw_status
			...
			if(fts_ctpm_fw_upgrade_with_fs(ImageBuffer,fullFileLength,true) == 0)
			{
			...
			else
				{
					...
					private_ts->vendor_fw_ver_L = i_CTPM_FW[FW_VER_MIN_FLASH_ADDR];
					private_ts->vendor_config_ver = i_CTPM_FW[FW_CFG_VER_FLASH_ADDR];
					FW_doupdate_M=1;//20160302 fw_version update
				}
#ifdef HX_RST_PIN_FUNC
			himax_HW_reset(false,true);
#endif
			fw_status = 3; //update fw_status 20160119
			return 1;	
		}
	else
		fw_status = 3; //update fw_status 20160119
...
//========================== add FW update 2nd source 20160201
static int i_update_FW_DJN(void)
{
	unsigned char* ImageBuffer = i_DJN_FW;
        int fullFileLength = sizeof(i_DJN_FW);

	I("IMAGE FW_VER=%x,%x.\n",i_DJN_FW[FW_VER_MAJ_FLASH_ADDR],i_DJN_FW[FW_VER_MIN_FLASH_ADDR]);
        I("IMAGE CFG_VER=%x.\n",i_DJN_FW[FW_CFG_VER_FLASH_ADDR]);
        if (( private_ts->vendor_fw_ver_H < i_DJN_FW[FW_VER_MAJ_FLASH_ADDR] )
                || ( private_ts->vendor_fw_ver_L < i_DJN_FW[FW_VER_MIN_FLASH_ADDR] )
                || ( private_ts->vendor_config_ver < i_DJN_FW[FW_CFG_VER_FLASH_ADDR] )
                || ( himax_calculateChecksum(false) == 0 ))
                {
                        fw_status = 2; //20160119 update fw_status
#ifdef HX_RST_PIN_FUNC
                        ...
                        {
                                        I("%s: TP upgrade OK\n", __func__);
                                        private_ts->vendor_fw_ver_H = i_DJN_FW[FW_VER_MAJ_FLASH_ADDR];
                                        private_ts->vendor_fw_ver_L = i_DJN_FW[FW_VER_MIN_FLASH_ADDR];
                                        private_ts->vendor_config_ver = i_DJN_FW[FW_CFG_VER_FLASH_ADDR];
					FW_doupdate_S=1;//20160302 fw_version update
                        }
#ifdef HX_RST_PIN_FUNC
                        himax_HW_reset(false,true);
#endif
...
static void himax_read_TP_info(struct i2c_client *client)
{
	...
	FW_H = private_ts->vendor_fw_ver_H;
	FW_L = private_ts->vendor_fw_ver_L;
	FW_CON = private_ts->vendor_config_ver;
	//20160302 fw_version update
}
...
static int himax_parse_wake_event(struct himax_ts_data *ts)
{
	...
	if(buf[GEST_PTLG_ID_LEN] == GEST_PTLG_HDR_ID1 && 
			buf[GEST_PTLG_ID_LEN+1] == GEST_PTLG_HDR_ID2)
	{
		gest_len = buf[GEST_PTLG_ID_LEN+3];
		gest_point = buf[GEST_PTLG_ID_LEN+2];

		I("gest_len = %d ",gest_len);
		I("gest_point= %d",gest_point);
				...
				if (ts->pdata->virtual_key)
				{
					...
					if (ts->protocol_type == PROTOCOL_TYPE_A) {
						...
						input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR,
							100);
						/*20160225 mask for zoom in and out
	...
	if(ts->pdata->proximity_bytp_enable){
		...
		if (hx_point_num != 0 ) {
			if(vk_press == 0x00)
					...
					for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
						if (((finger_pressed >> loop_i) & 1) == 1) {
							...
							if (ts->protocol_type == PROTOCOL_TYPE_A)
							{
								...
								//input_mt_sync(ts->input_dev); 20160225 mask for zoom in and out
							}
							else
							{
								ts->last_slot = loop_i;
							...
							// 20160225 mask for zoom in and out start
							...
							// 20160225 mask for zoom in and out end
							if (!ts->first_pressed)
							{
								ts->first_pressed = 1;
								I("S1@%d, %d\n", x, y);
				...
				ESD_COUNTER = 0;
#endif
			//input_report_key(ts->input_dev, BTN_TOUCH, finger_on); 20160225 mask for zoomin and zoomout
			input_sync(ts->input_dev);
...
#ifdef HX_TOUCH_REC
        err_need2recover:
	I("%s: Do err_need2recover to check.\n", __func__);
        while(checktouch>0)
        {
                if(check_touchData()==1)  //check data which is like MXPT,RX NUM,TX NUM and so on... if it's need torecover
                {
	...
	size_t len, loff_t *pos)
{
	...
	if (buf[0] == 'm' && buf[1] == 'r' && buf[2] == ':') {
		...
		if (buf[2] == 'x') {
			...
			//for (loop_i = 0; loop_i < 128; loop_i++) {
			for (loop_i = 0; loop_i < 16; loop_i++) { ////20160225 for KPI: CID 90879
				if (buf[base] == '\n') {
					if (buf[0] == 'w') {
						if (config_bank_reg) {
							outData[0] = 0x14;
...
#ifdef HX_TP_FWVERSION_CHECK
//========================================== 20160119 add version node start

static ssize_t himax_fwversion_read(struct file *file, char *buf,
	size_t len, loff_t *pos)
{
       size_t ret = 0;

       //himax_read_FW_ver(true); 20160302 mask
       if(!HX_PROC_SEND_FLAG)
       {
		ret += sprintf(buf + ret, "FW_VER = ");
               //ret += sprintf(buf + ret, "0x%2.2X, %2.2X \n",private_ts->vendor_fw_ver_H,private_ts->vendor_fw_ver_L);
...
static struct file_operations himax_proc_fwversion_ops =
{
	.owner = THIS_MODULE,
	.read = himax_fwversion_read,
};
...
//      4: Fail
	size_t ret = 0;
        printk( "[Himax] get_fw_state() fw_status : %d\n", fw_status);
...
#endif
//========================================== 20160201 add get TPID_CHECK start
#ifdef HX_TPID_CHECK
...
#endif
//========================================== 20160201 add get TPID_CHECK end
...
static int himax_chip_self_test(void)
{
	...
	*/
	//===20160216 mask for Speed Fw
	read_result_again: //20160216 for Speed Fw
	...
	if (valuebuf[0]==0xAA) {
		...
		pf_value = 0x0;
	} else if((valuebuf[0] == 0xf1) || (valuebuf[0] == 0xf2) || (valuebuf[0] == 0xf3)){//20160216 for Speed Fw
		E("[Himax]: self-test fail\n");
		pf_value = 0x1;
	} else {//20160216 for Speed Fw
...
{	
	...
	int err = -ENOMEM;
	int up_flag;//20160119 for FW_STATE
	struct himax_ts_data *ts;
	struct himax_i2c_platform_data *pdata;
...
himax_gpio_power_config(ts->client,pdata);//20160517 add
...
#ifdef HX_AUTO_UPDATE_FW
//==================================================================== add start
	mtk_wdt_enable(WK_WDT_DIS); //20160119 reboot every 30s issue
	fw_status = 1; //update fw_status 20160119

	if(CTP_ID_g==1)
		up_flag=i_update_FW();
	else
		up_flag=i_update_FW_DJN();
	...
	}else if(FW_doupdate_S==1){
		FW_H=i_DJN_FW[FW_VER_MAJ_FLASH_ADDR];
		FW_L=i_DJN_FW[FW_VER_MIN_FLASH_ADDR];
		FW_CON=i_DJN_FW[FW_CFG_VER_FLASH_ADDR];
	}
	//20160302 fw_version update
...
/* 20160119 mask test
	if(i_update_FW()==false)
		...
		I("Have new FW=UPDATE=\n");
*/
	mtk_wdt_enable(WK_WDT_EN); //20160119 reboot every 30s issue
//=================================================================== add end
#endif
...
if(HX_ON_HAND_SHAKING)//chip on hand shaking,wait hand shaking
{
	...
	//himax_int_enable(ts->client->irq,0,true);20160517 mask
	disable_irq_nosync(ts->client->irq);//20160517
	...
	if(IS_ERR(tpd->reg))
        {
                 ...
                 printk("tpd regulator_get_voltage(%d) failed!\n",retval);
        }
        msleep(5);
	//20151215 end add
    retval = regulator_set_voltage(tpd->reg, 2800000, 2800000);
    if (retval != 0) {
    	I("Failed to set reg-vgp6 voltage: %d\n", retval);
    	return -1;
    }
