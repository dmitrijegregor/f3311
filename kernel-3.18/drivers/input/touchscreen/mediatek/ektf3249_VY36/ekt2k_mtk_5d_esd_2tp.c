...
#define	CHECK_HW_ID				0 // check HW ID
#define	CHECK_CTP_ID				0 // check CTP ID
...
static uint8_t file_fw_data_CPT_GLASS[] = {
  #include "VY36_Glass_FW5583_ID00BE_20151021.i" /* modify */
};
//add DP2 FW, end
...
static uint8_t file_fw_data_INX_GLASS[] = {
  #include "VY36_INX_3260_55C1_00D0_20151106.i" /* modify */
};
...
else if(SENSOR_OPTION == 0x00BF){//INX/PMMA
	  printk("[ELAN] Update_FW_One() USE (INX/PMMA) FW, skip\n");
	  goto Upd_Fail;
}
...
//mdelay(1500);
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "__hello_packet_handler: do polling int\n");
  rc = elan_ktf2k_ts_poll( client );
  if( rc < 0 )
  {
    //printk( "[elan] %s: Int poll failed!\n", __func__ );
    printk( "[elan] Int poll failed!\n");
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "__hello_packet_handler: Int poll failed!\n");
    RECOVERY = 0x80;
  ...
  if( rc < 0 ){
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "%s: read hello packet fail! rc = %d\n", __func__, rc);
	return rc;
  }
  ...
  if(( buf_recv[0] == 0x55 ) && ( buf_recv[1] == 0x55 ) && ( buf_recv[2] == 0x80)  && ( buf_recv[3] == 0x80 ))
  {
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "%s: into RECOVERY MODE!!! SENSOR_OPTION = 0x%04X\n", __func__, SENSOR_OPTION);
	RECOVERY = 0x80;
	return RECOVERY;
  }
  else if(( buf_recv[0] == 0x55 ) && ( buf_recv[1] == 0x55 ) && ( buf_recv[2] == 0x55)  && ( buf_recv[3] == 0x55 )){
	printk("[elan] get Hello Packet OK.\n");
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "%s: get Hello Packet OK.\n", __func__);
  }
  else{
	printk("[elan] get UNKNOWN Packet, return fail\n");
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "%s: get UNKNOWN Packet, return fail\n", __func__);
	return -1;
  }
  ...
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "__hello_packet_handler: return rc = %d\n", rc);
  return rc;
}
...
//add different TP source info when TOUCH up, start
	if(SENSOR_OPTION == 0x9999){//HS + TR
		...
		printk("[elan] (INX/PMMA) Touch up: X = %d, Y = %d\n", last_X, last_Y);
	}
	else if(SENSOR_OPTION == 0x00D0){//INX/Glass
...
struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "result_lines_probe = \n%s, size of result_lines_probe = %d, result_lines_index = %d\n", result_lines_probe, strlen(result_lines_probe), result_lines_index);//20151125
}
...
// ELAN_FW_VER_CPT_GLASS       0x5583
// ELAN_FW_VER_INX_PMMA       0x5501
// ELAN_FW_VER_INX_GLASS       0x5501

// ELAN_SENSOR_OPTION_DP1       0x9999
// ELAN_SENSOR_OPTION_CPT_PMMA       0x00BA
// ELAN_SENSOR_OPTION_CPT_GLASS       0x00BE
// ELAN_SENSOR_OPTION_INX_PMMA       0x00BF	//Innolux
...
//colby add end
  ...
  //mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
	...
	printk("[elan] ELAN_FW_VER_INX_PMMA = %04X, ELAN_SENSOR_OPTION_INX_PMMA = %04X\n", ELAN_FW_VER_INX_PMMA, ELAN_SENSOR_OPTION_INX_PMMA);
...
// TP FW check always PASS for DP2, end
 ...
 //check FW & Sensor Option
	switch(SENSOR_OPTION){
		...
		case ELAN_SENSOR_OPTION_INX_PMMA:// Innolux/PMMA
		{
			printk("[elan] get_ftm_fw_check() INX/PMMA\n");
			if(FW_VERSION == ELAN_FW_VER_INX_PMMA){
				//PASS
				return sprintf(buf, "Pass\n");
			}
			else{
				//FAIL
				return sprintf(buf,
				"fw check version FAIL:\n ELAN_FW_VER_INX_PMMA = 0x%04X, FW_VERSION = 0x%04X\n ELAN_SENSOR_OPTION_INX_PMMA = 0x%04X, SENSOR_OPTION = 0x%04X\n",
					ELAN_FW_VER_INX_PMMA,
					FW_VERSION,
					ELAN_SENSOR_OPTION_INX_PMMA,
					SENSOR_OPTION);
			}
		} break;
...
struct device_attribute *attr, char *buf)
{
	...
	else if(SENSOR_OPTION == 0x00BF && CTP_ID_g == 1){//INX/PMMA
		printk("[elan] IN/PMMA\n");
		return sprintf(buf, "IN/PMMA\n");
	}
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		ret = request_irq(private_ts->client->irq, (irq_handler_t) tpd_eint_interrupt_handler, IRQF_TRIGGER_FALLING,
				"TOUCH-eint", NULL);
		if (ret > 0) {
			ret = -1;
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "%s: irq:%d, debounce:%d-%d:\n", __func__, private_ts->client->irq, ints[0], ints[1]);
	return ret;
}
#endif
//colby add end
...
static int elan_power_on(struct elan_ktf2k_ts_data *ts, int on)
{
    int ret;
    printk("%s++\n", __func__);
    result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_power_on: enter\n");
    if (on)
    {
        ret = regulator_enable(tpd->reg);
        if (ret) {
            result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_power_on: Regulator vdd enable failed ret=%d\n", ret);
...
*/
    ...
    else
    {
        ...
        if(ret){
            result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_power_on: Regulator vdd disable failed ret=%d\n", ret);
...
*/
    ...
    result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_power_on: leave\n");
    printk("%s--\n", __func__);
    return ret;
}
...
static int elan_power_config(struct elan_ktf2k_ts_data *ts)
{
    ...
    printk("%s ++\n", __func__);
    result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_power_config: enter\n");
    tpd->reg = regulator_get(tpd->tpd_dev, "vtouch");
    if (IS_ERR(tpd->reg)) {
        ret = PTR_ERR(tpd->reg);
        result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_power_config: Regulator get failed vdd ret=%d\n", ret);
    ...
    if (regulator_count_voltages(tpd->reg) > 0) {
        ...
        if (ret) {
            result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_power_config: Regulator set_vtg failed vdd ret=%d\n", ret);
...
*/
    result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_power_config: leave\n");
    printk("%s --\n", __func__);
    return 0;
/*
reg_vcc_i2c_put:
...
static int elan_parse_dt(struct elan_ktf2k_ts_data *ts, struct elan_ktf2k_i2c_platform_data *pdata)
{
    ...
    if (!gpio_is_valid(pdata->intr_gpio)) {
        ...
        result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_parse_dt:gpio_irq value is not valid\n");
    }
    ...
    if (!gpio_is_valid(pdata->rst_gpio)) {
        ...
        result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_parse_dt:gpio_rst value is not valid\n");
    }
...
result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "probe enter\n");
...
//private_ts->pdata = pdata;
    ...
    if(client->dev.of_node)
    {
        ...
        //result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "do elan_parse_dt enter.\n");
        ret = elan_parse_dt(private_ts, pdata);
        //result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "do elan_parse_dt leave. ret = %d\n", ret);
        if (ret < 0) { 
	 ...
	 result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "intr_gpio = %d\nrst_gpio = %d\n", private_ts->intr_gpio, private_ts->rst_gpio);
	 private_ts->pdata = pdata;
    }
...
//colby add end
  ...
  //result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "ELAN enter tpd_probe.\n");
...
result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "g_HW_ID = %d\n", g_HW_ID);

g_CURRENT_MODE = get_ftm_pin();
printk("[elan] g_CURRENT_MODE = %d\n", g_CURRENT_MODE);
result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "g_CURRENT_MODE = %d\n", g_CURRENT_MODE);
#endif
// check HW ID end
...
//#if 0
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "set reset pin OK!\n");
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "config int pin OK!\n");
	//enable_irq(private_ts->client->irq);
#endif
//colby add end
...
//colby add start
  ...
  tpd_load_status = 1;
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "setup INT and RESET OK!!!\n");
//
//check CTP ID start
#if CHECK_CTP_ID
printk("[elan] read CTP_ID start\n");
  ...
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "CTP_ID_g = %d\n", CTP_ID_g);

printk("[elan] read CTP_ID end\n");
#endif
//check CTP ID end

  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "alloc DMA MODE enter\n");
#if defined( ELAN_DMA_MODE )
  gpDMABuf_va = (u8 *)dma_alloc_coherent( NULL, 4096, &gpDMABuf_pa, GFP_KERNEL ); //20151125 for 32bit
  ...
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "alloc DMA MODE fail...\n");
    goto EXIT;
  }
  else{
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "alloc DMA MODE OK!\n");  	
  	}
#endif
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "alloc DMA MODE leave\n");
...
//colby add end
  ...
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_ktf2k_ts_setup fail, go EXIT!!!\n");
    goto EXIT;
  }
...
#endif
  //mt_eint_unmask( CUST_EINT_TOUCH_PANEL_NUM );
  result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "elan_ktf2k_ts_setup OK!\n");
...
//
  ...
  if( IS_ERR( thread ))
  {
    ...
    result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "failed to create kernel thread: %d\n", retval);
    goto EXIT;
  }
...
/* Firmware Update */
  ...
  if( misc_register( &ts.firmware ) < 0 ){
    ...
    result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "misc_register failed!!\n");
    //goto EXIT;
    }
  else{
    printk("[elan] misc_register finished!!\n");
    result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "misc_register OK!\n");
    }
/* End Firmware Update */
...
// check sensor option/TP ID for choosing FW start
	...
	if(SENSOR_OPTION == 0x9999){//HS + TR
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "USE (HS - TR) FW\n");
	...
	//file_fw_data_CPT_PMMA
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "USE (CPT/PMMA - TR) FW\n");
	...
	//file_fw_data_CPT_GLASS
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "USE (CPT/Glass - TR) FW\n");
		...
		printk("[elan] USE (INX/PMMA) FW, skip\n");
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "USE (INX/PMMA) FW, skip\n");
		fw_status = 3; //update fw_status 20151020
		goto END_FW_UPDATE;
	}
	else if(SENSOR_OPTION == 0x00D0){//INX/Glass
	//file_fw_data_INX_GLASS
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "USE (INX/Glass) FW\n");
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "UNKNOWN Sensor Option, use (HS - TR) FW\n");
		New_FW_ID   = ( file_fw_data[0x7D67] << 8 ) | file_fw_data[0x7D66] ;
		New_FW_VER  = ( file_fw_data[0x7D65] << 8 ) | file_fw_data[0x7D64] ;
	}
#else
...
result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "FW_UPDATE_PROBE_CEI enter\n");
result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "FW_VERSION =  0x%X, New_FW_VER = 0x%X\n", FW_VERSION, New_FW_VER);
...
if(g_CURRENT_MODE == 1){
	...
	result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "IN CURRENT check mode, dont do FW update!\n");
	fw_status = 3; //update fw_status 20151020
	goto END_FW_UPDATE;
}
...
// check HW ID end
  ...
  //{
      if( New_FW_VER > ( FW_VERSION ))
      {
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "do FW update!\n");
  ...
  //}
	else if(RECOVERY==0x80)
	{
		...
		result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "In RECOVERY mode!\n");
		fw_status = 1; //update fw_status 20151020
		//Update_FW_One( client, RECOVERY );
		//fw_update_thread = kthread_run(Update_FW_in_Driver, NULL, "elan_update");
		//fw_update_thread = kthread_run(Update_FW_One, NULL, "elan_update");	// Thunder modify for update touch Firmware 20141229 //colby mask 20151001
    ...
    else
    {
      ...
      result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "No need to do FW update!\n");
      fw_status = 3; //update fw_status 20151020
    }
result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "FW_UPDATE_PROBE_CEI leave\n");
#endif
...
//add device node, end
    ...
    result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "probe OK!\n");
...
//colby add end
    ...
    result_lines_index += snprintf(&result_lines_probe[result_lines_index], 1024 - result_lines_index, "probe fail\n");
    return -1;
//error handling, end
}
...
#else
  	printk("[elan esd] esd reset enter\n");
