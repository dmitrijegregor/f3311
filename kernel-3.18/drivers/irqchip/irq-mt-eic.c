...
static unsigned int mt_eint_get_mask(unsigned int eint_num)
{
	...
	if (eint_num < EINT_MAX_CHANNEL) {
		...
		//dbgmsg 20160419
		    printk("Error in %s [EINT] num:%d is larger than EINT_MAX_CHANNEL\n",
		     __func__, eint_num);
		return 0;
	}
...
int mt_eint_get_polarity(unsigned int eint_num)
{
	...
	//dbgmsg("[EINT] %s :%lx, bit:%x, val:%x\n", __func__, base, bit, val); //20160419
	printk("[EINT] %s :%lx, bit:%x, val:%x\n", __func__, base, bit, val);
	if (val & bit)
		pol = MT_EINT_POL_POS;
	else
		pol = MT_EINT_POL_NEG;
...
static unsigned int mt_eint_get_debounce_cnt(unsigned int cur_eint_num)
{
	...
	if (cur_eint_num >= MAX_HW_DEBOUNCE_CNT)
		...
		default:
			deb = 0;
			/*pr_err*/printk("inv deb time in the EIN_CON, dbnc:%d, deb:%d\n"
				, dbnc, deb);
			break;
		}
	}
