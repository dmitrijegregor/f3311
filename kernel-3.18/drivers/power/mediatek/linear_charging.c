...
extern unsigned int bypass_bcct;
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		if ((g_temp_status == TEMP_ABOVE_POS_60)
			...
			battery_log(BAT_LOG_FULL,"CEI(K) WARM v_cc2topoff %d, v_recharge %d\n\r",v_cc2topoff_threshold, g_jeita_recharging_voltage);
		/*}*/
...
//CEI comments E
			...
			battery_log(BAT_LOG_FULL,"CEI(K) NORMAL v_cc2topoff %d, v_recharge %d, full current %d\n\r",v_cc2topoff_threshold, g_jeita_recharging_voltage, charging_full_current);
		}
		...
		if ((g_temp_status == TEMP_NEG_10_TO_POS_0 || g_temp_status == TEMP_BELOW_NEG_10)
			...
			battery_log(BAT_LOG_FULL,"CEI(K) COOL v_cc2topoff %d, v_recharge %d\n\r",v_cc2topoff_threshold, g_jeita_recharging_voltage);
		}
		...
		if ((g_temp_status == TEMP_BELOW_NEG_10)
			...
			battery_log(BAT_LOG_FULL,"CEI(K) COLD v_cc2topoff %d, v_recharge %d\n\r",v_cc2topoff_threshold, g_jeita_recharging_voltage);
		/*}*/
	} else {
		battery_log(BAT_LOG_CRTI,
...
//CEI comments S
...
#else
	{
		...
		if (battery_id_invalid){
			...
			battery_log(BAT_LOG_CRTI, "CEI(K)=>[BATTERY] battery_id_invalid %d, cv_voltage %d \r\n", battery_id_invalid, cv_voltage);			
		} else {
			battery_log(BAT_LOG_CRTI, "CEI(K)=>[BATTERY] do_jeita_state_machine(), cv_voltage =%d \r\n", cv_voltage);
	}

#ifdef CEI_ALIEN_BATTERY_PARAMETERS
	battery_log(BAT_LOG_CRTI,"CEI(K) g_fg_battery_id %d\n\r",g_fg_battery_id);
	if (g_fg_battery_id == 2) {
		g_jeita_recharging_voltage = ALIEN_RECHARGING_VOLTAGE;
		v_cc2topoff_threshold = ALIEN_V_CC2TOPOFF_THRES;
		charging_full_current = ALIEN_FULL_CURRENT;
		cv_voltage = ALIEN_CV_VOLTAGE;
		battery_log(BAT_LOG_CRTI,"CEI(K) Alien battery: g_fg_battery_id %d, v_cc2topoff %d, v_recharge %d, cv_voltage %d, full_current %d\n\r",g_fg_battery_id,v_cc2topoff_threshold,g_jeita_recharging_voltage,cv_voltage,charging_full_current);
	}
#endif
//CEI comments E
...
extern kal_bool bNSC;
extern unsigned long UnplugTime;
extern unsigned long LastUnplugTime;
extern unsigned long UnplugCount;
EXPORT_SYMBOL(bNSC);
EXPORT_SYMBOL(UnplugTime);
EXPORT_SYMBOL(LastUnplugTime);
EXPORT_SYMBOL(UnplugCount);
...
//CEI comments S 
	...
	if ((g_temp_status == TEMP_NEG_10_TO_POS_0) || (g_temp_status == TEMP_ABOVE_POS_60)) {
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d, status: %d\n",
...
//[Feature] 22871 Invalid and weak charger pop up indication S	
//need to revise after JEITA integrated
	if(bNSC)
		g_temp_CC_value = CHARGE_CURRENT_450_00_MA;
//[Feature] 22871 Invalid and weak charger pop up indication E
}
...
//CEI comment start//
	if(bypass_bcct){
		battery_log(BAT_LOG_CRTI, "[BATTERY] CEI(k) bypass bcct, g_temp_CC_value %d, g_bcct_flag %d, bypass_bcct %d\r\n",g_temp_CC_value,g_bcct_flag,bypass_bcct);
		return 1;
	}
//CEI comment end//
	...
	if (current_limit != -1) {
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] CEI(k) g_temp_CC_value %d, g_bcct_flag %d\r\n",g_temp_CC_value,g_bcct_flag);
...
//[Feature] 22871 Invalid and weak charger pop up indication E
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] Default CC mode charging : %d , bNSC = %d\r\n",
			    g_temp_CC_value, bNSC);
...
#ifdef CEI_ALIEN_BATTERY_PARAMETERS
		if (g_fg_battery_id == 2) {
			g_temp_CC_value = ALIEN_CHARGING_CURRENT;		
			battery_log(BAT_LOG_CRTI,"CEI(K) Alien battery g_fg_battery_id %d, g_temp_CC_value %d\n\r",g_fg_battery_id,g_temp_CC_value);
		}
#endif
//CEI comments E
	}
...
void pchr_turn_on_charging(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[BATTERY] pchr_turn_on_charging()!\r\n");
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
...
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT)
		battery_log(BAT_LOG_CRTI,"[BATTERY] pump express algorithm starts.\n");
		...
		if (get_usb_current_unlimited()) {
			...
			//CEI comments start S//
			//add dynamic debug node for ESTA test
			#ifdef ORG_VER
				if (g_bcct_flag == 1) {
			#else
				if(g_bcct_flag == 1 && !bypass_bcct) {
			#endif
			//CEI comments start E//
				battery_log(BAT_LOG_CRTI,
				...
				//CEI comments start S//
				battery_log(BAT_LOG_CRTI, "CEI(k)->[BATTERY] select_charging_current, g_bcct_flag = %d, bypass_bcct = %d \n",g_bcct_flag,  bypass_bcct);
				//CEI comments start E//
...
//CEI comment end//
			...
			battery_log(BAT_LOG_CRTI, "[BATTERY] CEI(K) CHARGING_CMD_SET_CV_VOLTAGE %d\r\n",cv_voltage);
			battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, &cv_voltage);
			#if defined(CONFIG_MTK_HAFG_20)
			g_cv_voltage = cv_voltage;
			#endif
	...
	//CEI comments S	
	battery_log(BAT_LOG_CRTI,"[BATTERY]CEI(K)=> pchr_turn_on_charging(), g_temp_CC_value =%d \r\n", g_temp_CC_value);
	battery_log(BAT_LOG_CRTI,"[BATTERY]CEI(K) chr_type %d, status %d, bat_vol %d, AvgI %d, AvgT %d, TOTchrtime %d.\n",
		BMT_status.charger_type,BMT_status.bat_charging_state,BMT_status.bat_vol,BMT_status.ICharging,BMT_status.temperature,BMT_status.total_charging_time);
	battery_log(BAT_LOG_FULL,"[BATTERY]CEI(K) g_temp_CC_value= %d, cc2topoff_vol= %d, full_current= %d.\n",
		g_temp_CC_value,v_cc2topoff_threshold,charging_full_current);
	//CEI comments E
...
PMU_STATUS BAT_PreChargeModeAction(void)
{
	...
	if ((BMT_status.UI_SOC == 100) && (bat_is_charging_full() == KAL_TRUE)) {
		BMT_status.bat_charging_state = CHR_BATFULL;
		BMT_status.bat_full = KAL_TRUE;
		g_charging_full_reset_bat_meter = KAL_TRUE;
		battery_log(BAT_LOG_CRTI, "[BATTERY] CEI(K) BAT_PreChargeModeAction set bat_charging_state = CHR_BATFULL\n\r");	
...
#else
	{
//CEI comments, UCH10 S
		if(BMT_status.charger_type == STANDARD_CHARGER){
			battery_log(BAT_LOG_CRTI, "[UCH10][BAT_PreChargeModeAction] Do not stop charging 1s, charger_type %d, g_temp_CC_value %d\n\r", BMT_status.charger_type, g_temp_CC_value);
		}else{
			...
			battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
			msleep(1000);
			battery_log(BAT_LOG_CRTI, "[BAT_PreChargeModeAction] stop charging 1s, charger_type %d\n\r", BMT_status.charger_type);
		}
//CEI comments, UCH10 E
	}
#endif
...
{
//CEI comments, decreasing Ichg by 50mA steps S
	int i_step = 0;
	kal_bool charging_enable = KAL_FALSE;
	kal_bool to_decrease_by_steps = KAL_FALSE;
	CHR_CURRENT_ENUM previous_ac_ichg = CHARGE_CURRENT_0_00_MA;
	CHR_CURRENT_ENUM hw_ichg_steps[] = {
...
//CEI comments, decreasing Ichg by 50mA steps E
...
//CEI comments, decreasing Ichg by 50mA steps S
		to_decrease_by_steps = (BMT_status.charger_type == STANDARD_CHARGER) && (g_temp_CC_value >= CHARGE_CURRENT_200_00_MA) && (g_temp_CC_value <= CHARGE_CURRENT_800_00_MA);
		if (to_decrease_by_steps) {
			previous_ac_ichg = g_temp_CC_value;

			battery_log(BAT_LOG_CRTI,"[UCH10][BAT_ConstantCurrentModeAction]initial status, g_temp_CC_value =%d, previous_ac_ichg %d\r\n", g_temp_CC_value, previous_ac_ichg);	

			for(i_step=0; i_step<7; i_step++){
				if( previous_ac_ichg < hw_ichg_steps[i_step] ){
					continue;
				}

				g_temp_CC_value = hw_ichg_steps[i_step];

				battery_charging_control(CHARGING_CMD_SET_CURRENT, &g_temp_CC_value);
				battery_log(BAT_LOG_CRTI,"[UCH10][BAT_ConstantCurrentModeAction][step%d], g_temp_CC_value =%d\r\n", i_step, g_temp_CC_value);
				battery_log(BAT_LOG_FULL,"[UCH10] 200ms start\r\n");
				msleep(200);
				if(g_temp_CC_value == CHARGE_CURRENT_300_00_MA){
					battery_log(BAT_LOG_CRTI,"[UCH10] msleep 1000ms start\r\n");
					msleep(1000);
					battery_log(BAT_LOG_FULL,"[UCH10] msleep 1000ms end\r\n");
					break;
				}
			}

			g_temp_CC_value = previous_ac_ichg;
			battery_log(BAT_LOG_CRTI,"[UCH10][BAT_ConstantCurrentModeAction]final status, g_temp_CC_value =%d\r\n", g_temp_CC_value);
		}else{
			//kal_bool charging_enable = KAL_FALSE;
			charging_enable = KAL_FALSE;
			...
			battery_log(BAT_LOG_CRTI, "[UCH10][BAT_ConstantCurrentModeAction] stop charging 1s, charger_type %d, g_temp_CC_value %d\n\r", BMT_status.charger_type, g_temp_CC_value);
		}
//CEI comments, decreasing Ichg by 50mA steps E
	}
#endif
