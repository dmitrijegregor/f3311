...
//CEI comments, UCH10 event S
#include <linux/switch.h>
#include <mt_gpio.h>
#include <linux/gpio.h>
#include <linux/irq.h>
#include <mach/gpio_const.h>
//CEI comments, UCH10 event E
...
int g_battery_thermal_throttling_flag = 1;//ori=3
...
int ovp_in = 0;
int ovp_out = 0;
int ovp_check_times = 0;
int ovp_switch_state = 0;
int ovp_debug_gpio_num = -1;
int ovp_debug_irq_num = -1;
int ovp_debug_eint_type = -1;

struct of_device_id ovp_of_match[] = {
	{ .compatible = "mediatek, OVP-eint", },
};
//CEI comments, UCH10 event E
...
//CEI comment start, FP22589 Prevent swelling
	int enable_llk;
//CEI comment end
	/* Dual battery */
	int status_smb;
	...
	int present_smb;
	int adjust_power;
/*KM, fake battery capacity and temperature for debug use S*/
	int fake_temp;
	int fake_capacity;
/*KM, fake battery capacity and temperature for debug use E*/
 //[Feature] 22871 Invalid and weak charger pop up indication S
	int weak_chgr;
	int invalid_charger;
 //[Feature] 22871 Invalid and weak charger pop up indication E
};
...
//[Feature] 22871 Invalid and weak charger pop up indication S
    	POWER_SUPPLY_PROP_weak_chgr,
    	POWER_SUPPLY_PROP_invalid_chgr,
//[Feature] 22871 Invalid and weak charger pop up indication E
};
...
static irqreturn_t ovp_handler(int irqno, void *data);
void ovp_check(void);
//CEI comments, UCH10 event E
...
/*KM, fake battery capacity and temperature for debug use S*/
		if (data->fake_capacity >= 0)
			val->intval = data->fake_capacity;
		else
/*KM, fake battery capacity and temperature for debug use E*/
...
//CEI comment end
	...
	case POWER_SUPPLY_PROP_adjust_power:
		...
		break;
//[Feature] 22871 Invalid and weak charger pop up indication S
   	 case POWER_SUPPLY_PROP_weak_chgr:
        	val->intval = data->weak_chgr;
        	printk("weak_chgr = %d\n",data->weak_chgr);
       	 	break;
   	 case POWER_SUPPLY_PROP_invalid_chgr:
        	val->intval = data->invalid_charger;
        	printk("invalid_charger = %d\n",data->invalid_charger);
       	 	break;
//[Feature] 22871 Invalid and weak charger pop up indication E
	default:
		ret = -EINVAL;
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENABLE_LLK:
		...
		battery_log(BAT_LOG_CRTI, "[KC][Swelling] battery_set_property sets enable_llk = %d\n", data->enable_llk);
		break;
	//CEI comment end
	default:
		return -EINVAL;
...
//CEI comment start, FP22589 Prevent swelling
		.set_property = battery_set_property,//KM, fake battery capacity and temperature for debug use
//CEI comment end
		.property_is_writeable = battery_property_is_writeable, //KM, fake battery capacity and temperature for debug use
		},
...
/*CEI comments full condition feature*/
//CEI comments S
#if 0
	if ((BMT_status.UI_SOC2 == 100) && (BMT_status.charger_exist == KAL_TRUE))
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_FULL;
#else
	if ((BMT_status.UI_SOC2 == 100) && (BMT_status.charger_exist == KAL_TRUE) && (bat_is_charging_full() == KAL_TRUE)){
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_FULL;
		battery_log(BAT_LOG_CRTI, "[mt_battery_update_EM] CEI(K) set BAT_STATUS as POWER_SUPPLY_STATUS_FULL.\n");
	}
#endif
//CEI comments E
...
extern kal_bool bNSC;//[Feature] 22871 Invalid and weak charger pop up indication 
...
//CEI comments, check JEITA stop charging S
...
//CEI comments, check JEITA stop charging E
#else
			bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_CHARGING;
#endif
		} else {
	...
	battery_log(BAT_LOG_CRTI, "[battery_update] CEI(K)=> BAT_STATUS %d\n",bat_data->BAT_STATUS);
...
//[Feature] 22871 Invalid and weak charger pop up indication S
	if (bNSC) 
		bat_data->weak_chgr = 1;
	else
		bat_data->weak_chgr = 0;

	if (BMT_status.charger_type == NONSTANDARD_CHARGER)
		bat_data->invalid_charger = 1;
	else 
		bat_data->invalid_charger = 0;
//[Feature] 22871 Invalid and weak charger pop up indication E
...
//CEI comment start//
	battery_log(BAT_LOG_CRTI,
			    "CEI(K)=> [kernel] mt_battery_GetBatteryData (V_BAT_SENSE)=%d, (V_I_SENSE)=%d\n", bat_vol, Vsense);
//CEI comment end//
}
...
//CEI comments start
//For ESTA test, disable safety timer in userdebug build
#ifndef DISABLE_SAFETY_TIMER
...
//CEI comment S
		if(BMT_status.charger_type == STANDARD_HOST|| BMT_status.charger_type == APPLE_0_5A_CHARGER){
			if(BMT_status.total_charging_time >= PC_MAX_CHARGING_TIME){
				battery_log(BAT_LOG_CRTI, "[BATTERY] charger type %d, Charging Over Time!\n", BMT_status.charger_type);
				status = PMU_STATUS_FAIL;
			}
		}else{
...
//CEI comment start, Prevent swelling, FP22589
int battery_swelling_chg_check(void)
{
	...
	if(bat_data != 0)
	{
		...
		if(batt_swelling_flag == 1)
		{
			...
			if(bat_data->BAT_CAPACITY <= 40)
			{
				...
				//reset charging state to CHR_PRE for first time
				if( (BMT_status.bat_charging_state != CHR_PRE) && (BMT_status.bat_charging_state != CHR_CC))
				{
					battery_log(BAT_LOG_CRTI, "CEI(K)=> SWELLING: BAT_CAPACITY<=40, BAT STATE=%d, set to CHR_PRE(%d) \n", BMT_status.bat_charging_state, CHR_PRE);
					BMT_status.bat_charging_state = CHR_PRE;
				}
			}
			else if(bat_data->BAT_CAPACITY >= 60)
			{
				battery_log(BAT_LOG_CRTI, "CEI(K)=> SWELLING: BAT_CAPACITY>=60, enable_chg is set to 0.\n");
				enable_chg = 0;
			}
			else//BAT_CAPACITY: 41~59%
			{
				//Do nothing
				battery_log(BAT_LOG_CRTI, "CEI(K)=> SWELLING: BAT_CAPACITY is 41~59, do nothing.\n");
			}
		}
		else
		{
			...
			{
				battery_log(BAT_LOG_CRTI, "CEI(K)=> SWELLING: batt_swelling_flag 1->0, set to CHR_PRE\n");
				BMT_status.bat_charging_state = CHR_PRE;
			}
		}
	}
	...
	battery_log(BAT_LOG_CRTI, "CEI(K)=> SWELLING: enable_llk=%d, enable_chg=%d, cap=%d\n", batt_swelling_flag, enable_chg,  battery_cap);
...
//Prevent swelling, FP22589 S
	if (battery_swelling_chg_check() != 1) {
		BMT_status.bat_charging_state = CHR_ERROR;
		battery_log(BAT_LOG_CRTI, "CEI(K)=> SWELLING: mt_battery_CheckBatteryStatus returns CHE_ERROR(%d).\n", BMT_status.bat_charging_state);
		return;
	}
//Prevent swelling, FP22589 E
...
static irqreturn_t ovp_handler(int irqno, void *data)
{
	...
	ovp_event = 1;
	battery_log(BAT_LOG_CRTI, "[ovp_handler][UCH10] ovp_event = %d\n", ovp_event);
	return res;
}
...
void ovp_check(void){
	...
	ovp_check_times++;
	is_ovp = (ovp_event == 1) && (ovp_out == 1) && (ovp_in == 1);

	if(ovp_out == 1){
		ovp_switch_state = 0;
		switch_set_state(&sdev, ovp_switch_state);
		battery_log(BAT_LOG_CRTI, "[ovp_check][UCH10] switch_set_state %d\n", ovp_switch_state);
	}

	if( ovp_check_times == 1 ){
		battery_log(BAT_LOG_CRTI, "[ovp_check][UCH10]check later, ovp_switch_state %d, ovp_event %d, ovp_in %d, ovp_out %d, ovp_check_times %d\n", ovp_switch_state, ovp_event, ovp_in, ovp_out, ovp_check_times);
	}else if( is_ovp && (ovp_check_times == 2) ){
		ovp_event = 0;
		ovp_in = 0;
		ovp_out = 0;
		ovp_switch_state = 1;
		switch_set_state(&sdev, ovp_switch_state);
		battery_log(BAT_LOG_CRTI, "[ovp_check][UCH10]is ovp, ovp_switch_state %d, ovp_event %d, ovp_in %d, ovp_out %d, ovp_check_times %d\n", ovp_switch_state, ovp_event, ovp_in, ovp_out, ovp_check_times);
	}else{
		ovp_event = 0;
		ovp_in = 0;
		ovp_out = 0;
		ovp_check_times = 0;
		battery_log(BAT_LOG_CRTI, "[ovp_check][UCH10]reset ovp, ovp_switch_state %d, ovp_event %d, ovp_in %d, ovp_out %d, ovp_check_times %d\n", ovp_switch_state, ovp_event, ovp_in, ovp_out, ovp_check_times);
	}
}
//CEI comments, UCH10 event S
...
//CEI comments, UCH10 event S
			ovp_in = 1;
			battery_log(BAT_LOG_CRTI, "[do_chrdet_int_task][UCH10] charger exist, ovp_in=%d\n", ovp_in);
//CEI comments, UCH10 event E
...
//CEI comments, UCH10 event S
			ovp_out = 1;
			battery_log(BAT_LOG_CRTI, "[do_chrdet_int_task][UCH10] charger not exist, ovp_out=%d\n", ovp_out);
//CEI comments, UCH10 event E
		}
...
//MTK add start
		//CEI comments, remove charging in do_chrdet_int_task for UCH10 charger
		//pchr_turn_on_charging();
		battery_log(BAT_LOG_FULL, "[do_chrdet_int_task] not to call pchr_turn_on_charging().\n");
//MTK add end

//CEI comments, UCH10 event S
		battery_log(BAT_LOG_CRTI, "[do_chrdet_int_task][UCH10] finally, ovp_switch_state %d, ovp_event %d, ovp_in %d, ovp_out %d\n", ovp_switch_state, ovp_event, ovp_in, ovp_out);
...
//CEI comments, UCH10 event S
	ovp_check();
	battery_log(BAT_LOG_CRTI, "[BAT_thread][UCH10] ovp_debug_gpio_num=%d, ovp_debug_irq_num=%d, ovp_debug_eint_type=%d\n", ovp_debug_gpio_num, ovp_debug_irq_num, ovp_debug_eint_type);
//CEI comments, UCH10 event E
...
//CEI comments, UCH10 event S
	node = of_find_matching_node(node, ovp_of_match);
    if (node) {
        of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
        of_property_read_u32_array(node, "interrupts", ints1, ARRAY_SIZE(ints1));
        gpiopin = ints[0];
        headsetdebounce = ints[1];
        ovp_eint_type = ints1[1];
        gpio_set_debounce(gpiopin, headsetdebounce);
        ovp_irq = irq_of_parse_and_map(node, 0);
        ret = request_irq(ovp_irq, ovp_handler, IRQF_TRIGGER_FALLING, "OVP-eint", NULL);

		ovp_debug_gpio_num = gpiopin;
		ovp_debug_irq_num = ovp_irq;
		ovp_debug_eint_type = ovp_eint_type;

        if (ret != 0) {
            battery_log(BAT_LOG_CRTI, "[battery_probe][UCH10]EINT IRQ LINE NOT AVAILABLE\n");
        } else {
            battery_log(BAT_LOG_CRTI, "[battery_probe][UCH10] set EINT finished, ovp_irq=%d, headsetdebounce=%d\n", ovp_irq, headsetdebounce);
        }
    } else {
        battery_log(BAT_LOG_CRTI, "[battery_probe][UCH10] can't find compatible node\n");
    }
//CEI comments, UCH10 event E
...
//CEI comments, UCH10 event S
static int uch10_switch_dev(void)
{
    int state, ret;
    sdev.name = "invalid_charger";
    ret = switch_dev_register(&sdev);
    if (ret < 0){
		battery_log(BAT_LOG_CRTI, "[uch10_switch_dev] switch_dev_register error!\n");
        return ret;
    }
    state = 0;
...
//CEI comments, UCH10 event S
    if(uch10_switch_dev())
    {
		battery_log(BAT_LOG_CRTI, "[%s] failed to create uch10 switch\n", __func__);
        return -ENODEV;
    }
//CEI comments, UCH10 event E
