...
#ifdef CEI_ALIEN_BATTERY_PARAMETERS
...
int g_battery_id_voltage[]={990, 1800, -1};
...
void fgauge_get_profile_id(void)
{
...
#ifndef CEI_ALIEN_BATTERY_PARAMETERS
	int id = 0;
#else
	//bat_id index_0 for 15Kohm, index_1 for 330Kohm, index_2 for others/alien battery.
	int battery_id_0_v_range[]={300, 400};//Rid=15k, id_volt ranges from 0.3V to 0.4V
	int battery_id_1_v_range[]={1300, 1700};//Rif=330k, id_volt ranges from 1.3V to 1.7V
	int hw_type = CEI_HW_UNKNOWN;

	hw_type = get_cei_hw_id();
	switch(hw_type) {
		case CEI_HW_EVT1://PDP1
		case CEI_HW_EVT2://PDP2
		case CEI_HW_DVT1://SP
			/*Battery ID spec for PDP1, PDP2, SP.*/
			battery_id_0_v_range[0] = 600;
			battery_id_0_v_range[1] = 800;
			battery_id_1_v_range[0] = 1400;
			battery_id_1_v_range[1] = 1800;
			break;
		default://Use default spec after phase AP			
			break;
	}

	bm_print(BM_LOG_CRTI, "CEI(K) hw_type = %d\n",hw_type);
	bm_print(BM_LOG_CRTI, "CEI(K) BAT_ID_0=(%d,%d), BAT_ID_1=(%d,%d)\n",
		battery_id_0_v_range[0],battery_id_0_v_range[1],
		battery_id_1_v_range[0],battery_id_1_v_range[1]);	
#endif
	...
	for(i = 0; i < BATT_ID_MAX_RETRY; i++)
	{
		...
		if (ret != 0)
		{
			bm_print(BM_LOG_CRTI, "CEI(K)=> [fgauge_get_profile_id]id_volt read fail\n");
			id_volt_read_fail_flag = 1;
		}
		else
		{
			bm_print(BM_LOG_CRTI, "CEI(K)=> [fgauge_get_profile_id]id_volt = %d\n", id_volt);
			id_voltage_read = id_volt;
		}

		if ((sizeof(g_battery_id_voltage) / sizeof(int)) != TOTAL_BATTERY_NUMBER) {
			bm_print(BM_LOG_CRTI, "CEI(K)=> [fgauge_get_profile_id]error! voltage range incorrect!\n");
			id_volt_range_incorrect = 1;
			return;
		}

#ifdef CEI_ALIEN_BATTERY_PARAMETERS
		if (id_volt >= battery_id_0_v_range[0] && id_volt <= battery_id_0_v_range[1]){
			g_fg_battery_id = 0;
			batt_init_done = 1;
		} else if (id_volt >= battery_id_1_v_range[0] && id_volt <= battery_id_1_v_range[1]){
			g_fg_battery_id = 1;
			batt_init_done = 1;
		} else {
			g_fg_battery_id = 2;
		...
		battery_log(BAT_LOG_CRTI,"CEI(K) [fgauge_get_profile_id], g_fg_battery_id %d, id_volt %d\n\r",g_fg_battery_id,id_volt);
#else
...
#endif
		bm_print(BM_LOG_CRTI, "CEI(K)=> [fgauge_get_profile_id]batt_init_done = %d\n", batt_init_done);
...
void get_profile_id_and_volt(int *batt_id, int *batt_id_volt)
{
	...
	*batt_id_volt = 0;
	...
	if (ret != 0)
		bm_print(BM_LOG_CRTI, "[get_profile_id_and_volt]CEI(K)=> id_volt read fail\n");
	else
		bm_print(BM_LOG_CRTI, "[get_profile_id_and_volt]CEI(K)=> id_volt = %d\n", id_volt);

	*batt_id_volt = id_volt;

	if ((sizeof(g_battery_id_voltage) / sizeof(signed int)) != TOTAL_BATTERY_NUMBER) {
		bm_print(BM_LOG_CRTI, "[get_profile_id_and_volt]CEI(K)=> error! voltage range incorrect!\n");
		return;
	}
	...
	bm_print(BM_LOG_CRTI, "[get_profile_id_and_volt]CEI(K)=> Battery id (%d)\n", fg_battery_id);

	*batt_id = fg_battery_id;
}
//CEI comments of Battery_ID E
...
//CEI comment start//
...
#else
	batt_meter_table_cust_data.battery_profile_t0_size = sizeof(battery_profile_t0[g_fg_battery_id])
	/ sizeof(BATTERY_PROFILE_STRUCT);

	memcpy(&batt_meter_table_cust_data.battery_profile_t0,
			&battery_profile_t0[g_fg_battery_id],
			sizeof(battery_profile_temperature));

	batt_meter_table_cust_data.battery_profile_t1_size = sizeof(battery_profile_t1[g_fg_battery_id])
	/ sizeof(BATTERY_PROFILE_STRUCT);
	...
	batt_meter_table_cust_data.battery_profile_t2_size = sizeof(battery_profile_t2[g_fg_battery_id])
	/ sizeof(BATTERY_PROFILE_STRUCT);
	...
	batt_meter_table_cust_data.battery_profile_t3_size = sizeof(battery_profile_t3[g_fg_battery_id])
	/ sizeof(BATTERY_PROFILE_STRUCT);
	...
	bm_print(BM_LOG_CRTI, "CEI(K)=> r_profile [0]=%d %d %d %d, r_profile[%d]=%d %d %d %d, size=%d %d %d %d\n",
	...
	{
		bm_print(BM_LOG_CRTI, "CEI(K)=> batt_meter_cust_data.q_max_pos_50 = %d, batt_meter_cust_data.q_max_pos_25 = %d, batt_meter_cust_data.q_max_pos_0 = %d, batt_meter_cust_data.q_max_neg_10 = %d\n",
		...
		bm_print(BM_LOG_CRTI, "CEI(K)=> batt_meter_cust_data.q_max_pos_50_h_current = %d,	batt_meter_cust_data.q_max_pos_25_h_current = %d, batt_meter_cust_data.q_max_pos_0_h_current = %d, batt_meter_cust_data.q_max_neg_10_h_current = %d\n",
...
//CEI comment start//
	bm_print(BM_LOG_CRTI, "CEI(K)=> [force_get_tbat] id_voltage_read=%d, read_fail_flag=%d, range_incorrect=%d, retry_counter=%d, battery_id = %d\n", id_voltage_read, id_volt_read_fail_flag, id_volt_range_incorrect, batt_id_retry_counter, g_fg_battery_id);
	bm_print(BM_LOG_CRTI, "CEI(K)=> shutdown_gauge0 = %d, shutdown_gauge1_xmins = %d, shutdown_gauge1_mins = %d\n", batt_meter_cust_data.shutdown_gauge0, batt_meter_cust_data.shutdown_gauge1_xmins, batt_meter_cust_data.shutdown_gauge1_mins);
//CEI comment end//
...
static ssize_t show_batt_id_adc(struct device *dev,struct device_attribute *attr, char *buf)
{
   ...
   bm_print(BM_LOG_CRTI, "[FG] [show_batt_id_adc] CEI(K)=> batt_id_volt : %d\n", batt_id_volt);
   return sprintf(buf, "%d\n", batt_id_volt);
}
...
static ssize_t show_batt_id_info(struct device *dev,struct device_attribute *attr, char *buf)
{
   ...
   bm_print(BM_LOG_CRTI, "[FG] [show_batt_id_info] CEI(K)=> batt_id_volt : %d, batt_id : %d, g_fg_battery_id : %d, g_battery_id_voltage[]=%d %d %d\n", batt_id_volt, batt_id, g_fg_battery_id, g_battery_id_voltage[0], g_battery_id_voltage[1], g_battery_id_voltage[2]);
...
static ssize_t show_CHDrvVersion(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	bm_print(BM_LOG_CRTI, "[FG] [show_CHDrvVersion] CEI(K)=> CHDrvVersion = %s\n", chgdrv_ver);
...
unsigned int bypass_bcct = 0;
static ssize_t show_bypass_bcct(struct device *dev,struct device_attribute *attr, char *buf)
{
	bm_print(BM_LOG_CRTI, "[FG] [show_bypass_bcct] CEI(K)=> bypass_bcct = %d\n", bypass_bcct);
   return sprintf(buf, "%d\n", bypass_bcct);
}

static ssize_t store_bypass_bcct(struct device *dev,struct device_attribute *attr, const char*buf, size_t size)
{
	if (0 == kstrtoint(buf, 10, &bypass_bcct)) {
		bm_debug("[FG] store_bypass_bcct: %d\n", bypass_bcct);
	} else {
		bm_debug("[FG] format error!\n");
	}
   return size;
}
static DEVICE_ATTR(bypass_bcct, 0664, show_bypass_bcct, store_bypass_bcct);
...
static ssize_t show_BMTbat_vol(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	bm_print(BM_LOG_CRTI, "[FG] [show_BMTbat_vol] CEI(K)=> BMT_status.bat_vol = %u\n", temp_bat_vol);
...
//CEI comments start//
	...
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_bypass_bcct);
//CEI comments end//
...
{
	bm_notice("CEI(K) battery_meter_suspend START!\n");
	/* -- hibernation path */
...
#endif
	bm_notice("CEI(K) battery_meter_suspend END!\n");
	return 0;
}
...
static int battery_meter_resume(struct platform_device *dev)
{
	...
	bm_notice("CEI(K) battery_meter_resume START!\n");
#ifdef MTK_POWER_EXT_DETECT
	if (KAL_TRUE == bat_is_ext_power())
		return 0;
#endif
...
#endif
#endif
	bm_notice("CEI(K) battery_meter_resume END!\n");
	return 0;
}
