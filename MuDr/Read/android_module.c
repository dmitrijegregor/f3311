...
int hooked_open(const char* path, int flags, mode_t mode) {
  ...
  if(uid != SYSTEM_UID)
    printk(KERN_EMERG "Hooked open function inside [ %d ] [ %d ]! path: %s flags: %d other: %d\n", (int) uid, (int) original_getpid(), path, flags, mode);
  ...
  if(uid == TARGET_UID) {
    ...
    if(strcmp(path, "/system/lib/libdvm.so") == 0) {
      printk(KERN_EMERG "Redirecting file to local version\n");
...
ssize_t hooked_read(int fd, char *buf, size_t count) {
  ...
  if(uid == TARGET_UID) {
    printk(KERN_EMERG "Hooked read function inside [ %d ] [ %d ]! fd: %d count: %d\n", (int) uid, (int) original_getpid(), fd, count);
  }
...
{
  printk(KERN_EMERG "open module unloaded.\n");
